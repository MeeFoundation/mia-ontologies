...
| `c:formGraph` | `c:FormGraph` | 1+ (required) on a live `c:Form`, capped in practice at the cell's own member count, per tool (see [Tools](#tools) above) | The graphs beneath one tool, one per claiming member; a different range from `c:member`, since a tool's `c:formTopic` need not be a PDN-mappable identity |
