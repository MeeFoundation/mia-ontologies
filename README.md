...
| `c:topic` | `c:SCGraph` | 0 on a bare `MemberCell`; 1+ (required, no upper bound) once the cell is typed `c:TopicCell` | One or more additional graphs beyond the `c:member` baseline |
