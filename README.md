...
A `c:ACell` is the instantiated facet of an abstract `c:Cell`. It carries `c:memberCount`, `c:creator`, `c:memberTopics`, `c:otherTopics`, and `c:shape`. Every `c:Cell` individual is always also typed `c:ACell` — there is no bare tree-position-only cell with no member content; a purely organizational cell with nothing substantive to say still carries a minimal stub `c:memberTopics` entry (claimed by and about `:Self`) rather than omitting member content altogether.
...
The blue text at the left of the cell displays the cell's derived subject(s) — not a stored value, but computed the same way for every cell regardless of member count: if the cell has a `c:otherTopics` value, that topic's own subject is the answer (e.g. `Employee`, `Friends`); otherwise the answer is every distinct subject among the topic graphs pointed to by `c:memberTopics` — i.e. the cell's own active members (e.g. `People`, and `Bob Johnson`'s "Self, Bob"). A `TwoMember`/`ThreePlusMember` cell with no `c:otherTopics` is essentially about the connection between all of its members — this is why the `BHS` cell at the bottom (`ThreePlusMember`, no `c:otherTopics`) shows "BHS, Bob, Self", not "BHS" alone.
