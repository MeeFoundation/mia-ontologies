...
A **tool** is an object a cell can carry — a form, a calendar, a drawing canvas — each with its own data format and its own contribution to the app's UI, sitting alongside the cell's unstructured note and its attachments. A cell carries zero or more, via `c:tool`; a cell with none is the ordinary case, not a lesser kind of cell. The ontology models only the data format; what a tool contributes to the interface is app-level, and [APP-BEHAVIOR.md](APP-BEHAVIOR.md) describes it.

<p align="center"><img src="images/cell-ontology/tool.png" alt="tool hierarchy"></p>

Three tool classes are defined. `c:FormTool` is the one whose format is settled: its data is graph DataBooks — one or more `c:FormGraph` values, whose fields the app renders as a form from the SHACL shape governing the tool (see [Form Fields from SHACL Shapes](APP-BEHAVIOR.md#form-fields-from-shacl-shapes) in APP-BEHAVIOR.md). `c:CalendarTool` and `c:CanvasTool` carry no properties of their own yet — they are defined so the extension point is visibly plural, and so that adding a tool later is a new subclass rather than a new property on `c:InstanceCell`.

All three tool properties — `c:formTopic`, `c:formGraph` and `c:formShape` — have domain `c:FormTool`, and `c:Tool` itself carries none. That follows from what a tool is rather than being an oversight: each kind brings its *own* data format, so there is nothing common to hoist onto the abstract class. A form's format is graphs conforming to a shape; a calendar's would be dated entries and a canvas's a drawing surface, neither of them a graph and neither worked out. A property that proves common once those two have formats of their own can be hoisted up then — cheap, where retracting one asserted too early is not.
...
There is no *cell-level* `v4.subject` field — who or what a cell's content is about is derived from `v4.member`/`v4.tool` rather than asserted independently; the `subject:` key that does appear sits inside each `v4.member` entry, naming that member rather than the cell's own subject (see [Graph Link Properties](#graph-link-properties) below).

#### Graph Link Properties

Each cell DataBook carries one or more `c:member` links (the required per-member baseline) and, on the cells that carry a tool, one or more `c:tool` links, each tool in turn linking whatever DataBook container(s) its own data format calls for:
...
| `c:tool` | `c:Tool` | 0..N — a cell with none is the ordinary case, and nothing caps how many it may carry (see [Tools](#tools) above) | Each tool brings its own data format and its own UI contribution; a form tool states, once, what its content is about (`c:formTopic`) |
