...
- **`c:origin`** — For `cat:CategoryDefine` nodes, the value of `cat:category` else nil. When a cell is shared with another member, the recipient's app can look at this value (if not nil) and use it as a hint as to which `cat:Node` in the recipient's app should point to it.
