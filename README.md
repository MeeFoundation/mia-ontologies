...
- **`shacl/persona-shacl.ttl`** — SHACL constraint rules for all `p:Person` individuals across all graphs. Validates properties including:
  ...
  - `:PrimaryCarePhysicianShape` — one of `cat:PrimaryCarePhysician`'s two `c:topicGraphShape` values, alongside `pshapes:ContactInfoShape` (CLAUDE.md's Check 32) — targets `p:Person` directly, since a physician has no reified document individual of their own. One optional property: `p:specialty` (e.g. Dr. Jane Starostina's own "Endocrinology"). Since both shapes target `p:Person`, Dr. Starostina's own topic graph (graph-25) is validated as a business-card profile in addition to her specialty
