...
A cell is the atomic unit of the tree the Mia software application manages for the user: a filesystem folder holding exactly one cell DataBook file, folder and file together forming one node — cells nest inside cells, forming the tree. It is a private, secure collaboration space that can be shared with one or more other members. These other members are usually other users, but may also be organizations that are compatible with the Personal Data Network.
...
A `c:ACell` is the instantiated facet of an abstract `c:Cell`. It carries `c:memberCount`, `c:creator`, `c:subject`, `c:memberTopics`, `c:otherTopics`, and `c:shape`. Every `c:Cell` individual is always also typed `c:ACell` — there is no bare tree-position-only cell with no member content; a purely organizational category cell with nothing substantive to say still carries a minimal stub `c:memberTopics` entry (claimed by and about `:Self`) rather than omitting member content altogether.
...
The blue text at the left of the cell displays the 1-2 subject(s) of the cell. If a `c:TwoMember` cell has a single subject this subject must be the subject of a topic graph in the `c:otherTopics`. And if `c:TwoMember` cell has two subjects they must be two distinct subjects of the 2..4 topic graphs pointed to by `c:memberTopics`. A TwoMember cell with two subjects is essentially about the connection/relationship between the two members.
...
|----------|-------|-------------|---------|
| `c:subject` | `xsd:anyURI` | 1 on `OneMember`/`ThreePlusMember`; 1..2 on `TwoMember` (required) | The resource(s) (typically a `p:Person`/`o:Organization`) the cell's relationship is about — not a topic container |
...
| `c:otherTopics` | `t:SCTopicGraph` | 0..N (optional), uniformly regardless of member count | Any number of additional topics beyond the `c:memberTopics` baseline — e.g. extra notes or supplementary claims not tied to a specific self-vs-other combination |
...
- One special file is the cell's **cell DataBook** (a `.databook.md` file with `type: cell-databook`) — see above for why a folder can never hold more than one. A cell DataBook's *filename* matches its own folder's name verbatim, with each space replaced by a hyphen (spaces are illegal inside a raw Turtle IRI) — its `id`, however, is a flat, opaque `cell-<NN>` value independent of the folder name (see CLAUDE.md's Check 9), the same pattern topic ids already use.
