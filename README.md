...
- **`cell:ACell`** — the *actual* (instantiated) facet of a `cell:Cell` (abstract). Carries `cell:parties`, `cell:creator`, and `cell:shape`. Every concrete party-count class (`cell:OneParty`/`cell:TwoParty`/`cell:ThreePlusParty`) is a subclass of `cell:ACell`, not `cell:Cell` directly.
...
**`cell-shacl.ttl`** — SHACL shapes for cell DataBook instances, split across three shapes matching `cell.ttl`'s facet split: `:CellShape` (target `cell:Cell`) constrains `cell:graph`, `cell:note`, and `cell:folder` to at most one value each, and `cell:sc-context` values, if any, to each be a `c:SCcontext` — with no cardinality distinction by party count, unlike the `cell:sbs`/`obs`/`sbo`/`obo` properties it replaced; `:TCellShape` (target `cell:TCell`) constrains `cell:templateShape` to at most one value; `:ACellShape` (target `cell:ACell`) constrains `cell:creator` to at most one value which, if present, must be a `p:Person`, `g:Group`, or `o:Organization`, `cell:parties` to exactly one value which must be the class `cell:OneParty`, `cell:TwoParty`, or `cell:ThreePlusParty`, and `cell:shape` to at most one value. `cell:templateShape`/`cell:shape` are deliberately not constrained to `sh:class sh:NodeShape`: the individuals they point at are only typed `sh:NodeShape` in `cell-templates-shacl.ttl`, which Tier 1 validation deliberately excludes from its merged-data run (see [Validation](#validation)), so that constraint would spuriously fail there.

### Cell Ontology Validation

Cell DataBook instances are validated by `cell-shacl.ttl`, for the same reason and via the same mechanism as [Category Ontology Validation](#category-ontology-validation) above: `parties`/`sc-context`/`graph`/`note`/`folder`/`creator`/`shape` exist solely as `mia.` YAML frontmatter fields on cell DataBooks, so `yaml-to-rdf.py` synthesizes the corresponding `cell:` triples (`rdf:type cell:Cell`/`cell:ACell`/the concrete party class, `cell:parties`, `cell:creator`, `cell:sc-context`, `cell:graph`, `cell:shape`) directly from frontmatter, letting `:CellShape`/`:ACellShape` actually fire against real instance data — see [Tier 1](#validation). (`cell:TCell` individuals live only in `cell-templates.ttl`, a plain `.ttl` file rather than a DataBook, so they need no such synthesis — see [Cell Ontology File](#cell-ontology-file).)
