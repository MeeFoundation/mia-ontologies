...
- **Tags** — an optional set of short text labels for finding the cell again later. Tags cut across the tree rather than placing the cell in it, so they are independent of the cell's category; see [Tags](APP-BEHAVIOR.md#tags) and [Finding Cells by Tag](APP-BEHAVIOR.md#finding-cells-by-tag) in APP-BEHAVIOR.md.
...
- **`c:builtInTag`** — a tag drawn from the app's own supplied set. A tag is a plain string retrieval label, orthogonal to `c:category`: a cell sits in at most one category but may carry any number of tags at once, and unlike `c:category` a tag records nothing about where a recipient's app should file the cell. Which of the three tag properties carries a given string is the whole of what records its kind — there is no reified tag individual and no separate kind marker. The set of strings the app ships is deliberately *not* enumerated in RDF: `shacl/cell-shacl.ttl`'s `:MemberCellShape` constrains a value only to a non-empty string, never to an `sh:in` list, since that set is an app-level concern that grows independently of this ontology (contrast `v:fuelType` and `pets:sex`, whose small closed vocabularies really are frozen via `sh:in`). Being built-in says where the string came from, not who may remove it — a member can drop one exactly as they can a `c:userTag`. Shared cell content: it propagates to every member's copy when the cell is shared, as the cell's own name and `c:category` value do. An `owl:DatatypeProperty`, domain `c:MemberCell`, range `xsd:string`, zero or more values, no fixed upper bound.

- **`c:userTag`** — a free-text tag the user minted themselves. Unconstrained beyond being a non-empty string, since the whole point of this kind is that the user is not choosing from any list: there is no vocabulary, scheme, or registry a value must belong to, which is exactly what separates it from `c:builtInTag`. Nothing prevents a user tag from coincidentally equaling a built-in one — the two are separate properties, so a cell carrying the same string under both is redundant but legal; only duplication *within* one kind is flagged (integrity.md's Check 36). Shared cell content, propagating like `c:builtInTag`. An `owl:DatatypeProperty`, domain `c:MemberCell`, range `xsd:string`, zero or more values, no fixed upper bound.

- **`c:integrationTag`** — a tag written by one member's own integration module — the Agent Bridge implementing an invited `s:AIAgentService` for a specific external service — for that module's own bookkeeping, rather than by a person. The value's internal structure is convention only: it is an opaque string today, and the example data's `"AppleContacts/Christmas_List"` merely spells out a `<module>/<label>` form by hand — nothing enforces the prefix and nothing parses it. That convention stands in for a future *structured* value, which is what will actually prevent two integration modules' labels from colliding; until then two modules picking the same bare string really would collide, and namespacing is the writing module's own responsibility. *Hidden*, in that the app never shows these in the cell's tag display alongside `c:builtInTag`/`c:userTag` values; hidden from display is not hidden from retrieval, and a tag search does match them. Alone among a cell's content, a `c:integrationTag` value is local to the instance that wrote it and is never propagated to another member when the cell is shared — the deliberate exception to the rule that a cell's content is kept in sync across every member's copy. That exception is a meaning one, not a privacy carve-out: each principal's device hosts and runs its own agent independently, so a bookkeeping string written by one member's module says nothing in a member's instance running a different integration, or none. Its consequence is that the same cell can legitimately carry different `c:integrationTag` values on either side of a share, with nothing merging the two. See [Tags](APP-BEHAVIOR.md#tags) in APP-BEHAVIOR.md. An `owl:DatatypeProperty`, domain `c:MemberCell`, range `xsd:string`, zero or more values, no fixed upper bound.

Note: none of the three is ever carried by a `c:TemplateCell` — their domain is `c:MemberCell` deliberately, so no tag is ever cloned into a new cell by [Lazy Instantiation](APP-BEHAVIOR.md#lazy-instantiation); every tag is applied to the real, already-instantiated cell afterwards. This is the opposite choice from `c:category`, whose domain really is `c:Cell` because a template does carry the concept it's a template for.
...
Two independent facts are drawn on each graph shape. **Shape** says what the graph is and, for a member, who that member is: a square for a `c:topic`, a circle for a `c:member` whose subject is a human (`p:Person`), and an octagon for a `c:member` whose subject is an `s:Service` — an invited AI agent, a backup service, or an organization's own service (see [Service Ontology](#service-ontology)). **Fill color** says only who claimed that graph: green for a graph claimed by anyone other than the self, and an outlined (unfilled) shape for one claimed by the self (the user). The two are deliberately orthogonal, so a shape's fill can be read without knowing anything about its claimant's type — an AI agent's claims are green like any other non-self party's, and the fact that it *is* an agent is carried by its octagon, not by its fill. For example, the `Bob Johnson` cell has four circles — two claimed by Bob, two claimed by Self. The `BHS` cell, a Topic Cell, has three circles (Self, Bob, and BHS's own member graphs) plus one square (BHS's organization profile, linked via `c:topic`). The `Medical Appointment` cell shows that a single topic can be claimed more than once: it has two squares alongside its two member circles — both about the same subject, one claimed by each side. A cell only ever has one topic; what `c:topic` holds is one graph per member asserting it. The `Kyoto Trip 2027` cell (see EXAMPLE.md's [Planning a Trip with an Agent](EXAMPLE.md#planning-a-trip-with-an-agent)) is the one box here drawn from real example data rather than generically, and the only one showing the gray/delegate fill: one of its three members is Alice's own invited travel agent, drawn as an octagon rather than a circle since its subject is an `s:AIAgentService` rather than a `p:Person`, and one of its three topic squares — all about the same trip — is claimed by that agent rather than by Alice or Dave. With one topic square per member, this cell also demonstrates `c:topic`'s real upper bound in practice (see [Topic Cell](#topiccell) above).
...
| `mia.builtInTag` | `c:builtInTag` | 0..N | A tag from the app's own supplied set (e.g. `"LoyaltyProgram"`, `"Emergency Contact"`). Shared cell content — propagates to every member's copy |
| `mia.userTag` | `c:userTag` | 0..N | A free-text tag the user minted (e.g. `"Ginger"`). Shared cell content |
| `mia.integrationTag` | `c:integrationTag` | 0..N | A tag written by this member's own integration module for its own bookkeeping (e.g. `"AppleContacts/Christmas_List"`). Never displayed, still searchable, and **local to this member's copy** — the one piece of cell content that does not propagate on a share |
...
| `c:topic` | `c:SCGraph` | 0 on a bare `MemberCell`; 1+ (required) once the cell is typed `c:TopicCell`, capped in practice at the cell's own member count (see [Topic Cell](#topiccell) above) | One or more additional graphs beyond the `c:member` baseline |
...
**`cell.ttl`** — The Cell ontology, defining:
  ...
  - *Datatype properties*: `c:isTopicCell` (domain `c:TemplateCell`, range `xsd:boolean`; required, exactly one value; flags whether Lazy Instantiation's clone of this template should be typed `c:TopicCell`); `c:builtInTag`, `c:userTag` and `c:integrationTag` (domain `c:MemberCell`, range `xsd:string`, zero or more values each, no fixed upper bound — a cell's tags, a retrieval dimension orthogonal to `c:category`'s single filing hint; see [MemberCell](#membercell) above). A tag's *kind* is carried entirely by which of the three properties holds it — there is no reified tag individual and no kind marker — and none of the three is ever asserted on a `c:TemplateCell`.
...
An `s:Service` is never a `c:creator` (see [Cell Ontology](#cell-ontology)) — a cell is always created by, and only ever owned by, a `p:Person` — but it can be a genuine `c:member` participant, and a `c:claimant` (see [Graphs](#graphs)) of the graphs it contributes, including a `c:topic` graph about whatever the cell's relationship concerns. Each member of a shared cell can independently bring their own AI agent: when a member invites theirs in, it becomes a real member, distinct from any agent a different member separately invites.

**Classes:**

- `s:Service` — the member-capable class for every cell participant that is not a person. Its three kinds below are deliberately not disjoint, so one real service can be more than one kind at once.
- `s:AIAgentService` — an AI agent (e.g. an LLM-based assistant such as ChatGPT) a person has invited to collaborate inside a shared cell. The only kind carrying `s:actsFor`.
- `s:BackupService` — a service that backs up a cell's content, joining that cell as a real member. Carries no `s:actsFor`: it is not any one member's delegate, it serves the cell itself.
...
`shacl/service-shacl.ttl` targets each class in turn. `:ServiceShape` (target `s:Service`) constrains `s:providedBy` and `s:actsFor` to at most one value each, an `o:Organization` and a `p:Person` respectively. `:AIAgentServiceShape` (target `s:AIAgentService`) raises `s:actsFor` to exactly one value; `:ServiceProviderShape` and `:BackupServiceShape` each raise `s:providedBy` to exactly one value. No shape forbids `s:actsFor` on the two kinds that don't use it — since the kinds are not disjoint, that would be unsatisfiable against `:AIAgentServiceShape`'s own minimum for any service typed both an AI agent and something else.
