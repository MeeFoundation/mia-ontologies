...
- **`c:declaresTool`** — names the tools a cell of this category is created with, each value a `c:Tool` node carrying its own `c:formShape` and nothing else — e.g. `ctpl:MedicalAppointmentTemplateCell` (which also carries `c:category cat:MedicalAppointment`) declares one `c:FormTool` carrying `mashapes:MedicalAppointmentRecordShape`. An `owl:ObjectProperty`, domain `c:TemplateCell`, range `c:Tool`, zero or more values, no fixed upper bound.
...
All three tool-content properties — `c:formTopic`, `c:formGraph` and `c:formShape` — have domain `c:FormTool`, and `c:Tool` itself carries none. That is deliberate rather than an oversight: the two stub classes have no content model, so asserting at the abstract level that every tool's content is about exactly one resource, or conforms to one shape, would commit them to something nothing has worked out. A property that proves common once they gain content can be hoisted up then — cheap, where retracting one asserted too early is not.
...
An `o:Organization` is never a `c:graphSubject`: an organization reaches a cell only as a `c:claimant` or a `c:formTopic`.

A `c:ToolGraph` carries no about-ness property of its own: what its content is about is the holding tool's single `c:formTopic` (see [Tools](#tools) above). Value is any resource IRI; unlike `c:graphSubject`, it carries no guarantee of being a PDN identity at all:
...
| `c:tool` | `c:Tool` | 0..N — a cell with none is the ordinary case, and nothing caps how many it may carry (see [Tools](#tools) above) | Each tool holds the cell's structured content and states, once, what that content is about (`c:formTopic`) |
| `c:formGraph` | `c:ToolGraph` | 1+ (required) on a live `c:FormTool`, capped in practice at the cell's own member count, per tool (see [Tools](#tools) above) | The graphs beneath one tool, one per claiming member; a different range from `c:member`, since a tool's `c:formTopic` need not be a PDN-mappable identity |
