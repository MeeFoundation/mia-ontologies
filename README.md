...
`c:folder` is a file path that points into a single folder hierarchy in local storage. Since a category's tree position IS that same filesystem folder now — there is no separate RDF individual to keep in sync — creating, renaming, or deleting a category is simply creating, renaming, or deleting the folder itself; there is no hierarchy to "adjust" separately.
...
|------------|-------------------|-------------|---------|
| `mia.origin` | `c:origin` | 0..1 | The matching `cat:Category` subclass whenever this cell's containing folder's own name matches a real one, as a class value (e.g. `"cat:Others"`); absent otherwise. A hint for a recipient's app when this cell is shared over PDN |
