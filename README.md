...
**`cell.ttl`** — The Cell ontology, defining:
  ...
  - *Datatype properties*: `c:isTopicCell` (domain `c:TemplateCell`, range `xsd:boolean`; required, exactly one value; flags whether Lazy Instantiation's clone of this template should be typed `c:TopicCell`); `c:userTag` and `c:integrationTag` (domain `c:MemberCell`, range `xsd:string`, zero or more values each, no fixed upper bound — a cell's tags, a retrieval dimension orthogonal to `c:category`'s single filing hint; see [MemberCell](#membercell) above). A tag's *kind* is carried entirely by which of the two properties holds it — there is no reified tag individual and no kind marker — and neither is ever asserted on a `c:TemplateCell`.
