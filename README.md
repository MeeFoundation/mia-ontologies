...
- `c:` for the `cell:` namespace (`http://mee.foundation/ontologies/cell#`) — also used for the graph-DataBook terms (`c:Graph`, `c:CGraph`, `c:MemberGraph`, `c:FormGraph`, `c:shape`, `c:claimant`, `c:graphSubject`) that live in `cell.ttl`
...
A `c:FormGraph` carries no about-ness property of its own: what its content is about is the holding tool's single `c:formTopic` (see [Tools](#tools) above). Value is any resource IRI; unlike `c:graphSubject`, it carries no guarantee of being a PDN identity at all:
...
| `c:formGraph` | `c:FormGraph` | 1+ (required) on a live `c:FormTool`, capped in practice at the cell's own member count, per tool (see [Tools](#tools) above) | The graphs beneath one tool, one per claiming member; a different range from `c:member`, since a tool's `c:formTopic` need not be a PDN-mappable identity |
...
The three graph shapes (see above) target `c:MemberGraph`/`c:FormGraph`, but that typing is itself only ever asserted from a graph's own `v4.member[]`/`v4.tool[].graph[]` entry, never as a literal `rdf:type` triple in the graph's own extracted Turtle body — and nothing in the entry marks its kind, since the list it sits in already does. The cell pass synthesizes it directly from the cell DataBook's frontmatter: a `v4.member` entry becomes `rdf:type c:MemberGraph` plus `c:claimant`/`c:graphSubject`, a graph under a `v4.tool` entry becomes `rdf:type c:FormGraph` plus `c:claimant` alone, each asserted on the graph's plain `id` (that same entry's own `id` value), not the `#graph`-suffixed `graph.named_graph` IRI — so all three shapes actually fire against real instance data; see [Validation](EXAMPLE.md#validation).
