...
The Cell class splits into two disjoint kinds: `c:TCell`, a reusable, class-level *template* cell, and `c:ACell`, an *actual* cell instantiated in a user's own tree. A cell is always exactly one of the two, never both — `cell-templates.ttl`'s template cells are typed solely `c:TCell`, and every cell in a user's own tree is typed solely `c:ACell`.

A cell is the atomic unit of the tree the Mia software application manages for the user: a filesystem folder holding exactly one cell DataBook file, folder and file together forming one node — cells nest inside cells, forming the tree.
