...
- `p:PetMedicationRecord` — label for graphs that carry a pet's list of medications. Not a subclass of `p:IdentityDocument`. Declared as `template: "persona:PetMedicationRecord"` in the graph's `mia.graphs[]` entry. SHACL shape `:PetMedicationRecordShape` (in `cell-templates-shacl.ttl`, alongside `cat:PetsMedical`'s template cell in `cell-templates.ttl`) targets the `p:PetMedicationRecord` record individual directly — the pet has no `p:Person` individual of its own, so the medication list is a property of the record, not of a person:
