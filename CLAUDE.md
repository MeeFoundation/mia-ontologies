...
def check_tree(root):
    ...
    #  explicitly allows this ("even if it contains nested category folders
    #  of its own") — so there is no ancestor-chain check here any more.
    ...
    # 2. A category folder holds exactly one cell-databook — a cell is
