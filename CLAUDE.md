...
If mismatches appear, the filename `(<containing-cell>)` segments are authoritative — update the cell DataBook's `primary`/`secondary` value to reference the correct topic (or rename the topic file if it was misnamed).
...
**Check 12 — `cell.ttl` matches `images/cell-ontology/cell.png`**: Current as of cell.ttl 3.11.0 — verified aligned. Redrawn for the context→topic rename: `cell:primary`/`cell:secondary`'s target box now correctly reads `t:SCtopic` (using README.md's `t:` shorthand for `topic:`), renamed from `context:SCcontext`. `Cell` (abstract, blue) carries only what's common to every cell regardless of facet: `note`, `folder`. `Cell` splits into two orthogonal facets, mirroring `category.ttl`'s Category/Node split (see Check 14): `TCell` (abstract, blue, the template facet) carries `templateShape` (to a `sh:NodeShape` box); `ACell` (abstract, blue, the actual/instantiated facet) carries `shape` (to a `sh:NodeShape` box), `creator` (to a union of `p:Person`/`o:Organization`/`g:Group`), `parties` (to `OneParty`/`TwoParty`/`ThreePlusParty` 1..1), `primary` (to a `t:SCtopic` box, 1..1 — renamed from `graph` in cell.ttl 3.10.0, domain moved here from `Cell`, range narrowed from plain `t:Topic`), and `secondary` (to the same `t:SCtopic` box, 0..N — renamed from `sc-context`, domain moved here from `Cell`). The party-composition hierarchy — `OneParty` and `MultiParty` (abstract) → `TwoParty`/`ThreePlusParty` — hangs off `ACell` now, not `Cell` directly; `TCell` has no subclasses of its own, since a template cell individual is instead multi-typed with both `TCell` and its `ACell`-lineage class (e.g. `OneParty`) — see `cell-templates.ttl`. `MultiParty` shows no arrows of its own, matching `cell:primary`/`cell:secondary`'s domain being the broader `cell:ACell` rather than `cell:MultiParty`. No arrow points from `Cell`, `TCell`, or `ACell` to any `cat:Node` box, matching that link now being asserted only on the category side (see Check 14). This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split, again for the 3.7.0 TCell/ACell split, and again for the 3.10.0 graph/sc-context → primary/secondary rename) — the party-composition hierarchy and its content-linking properties. Unlike Check 10 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask. After any change to `cell.ttl` or to this diagram, verify:

- **12a** — every property arrow shown off `Cell` (`note`, `folder`) has a corresponding `cell:` property in `cell.ttl` with `rdfs:domain cell:Cell`. Every arrow off `TCell` (`templateShape`) has `rdfs:domain cell:TCell`; every arrow off `ACell` (`shape`, `creator`, `parties`, `primary`, `secondary`) has `rdfs:domain cell:ACell` — `creator` and `parties` were narrowed here from `cell:Cell` in cell.ttl 3.7.0, and `primary`/`secondary` (renamed from `graph`/`sc-context`) in 3.10.0. `MultiParty` should show no arrows of its own — it carries no property, since `cell:primary`/`cell:secondary`'s domain is the broader `cell:ACell`. Each arrow's target type in the diagram must match the property's `rdfs:range` — `primary`'s and `secondary`'s are both `topic:SCtopic` (narrowed from plain `topic:Topic` for `primary`/`graph` in 3.10.0; renamed from `context:SCcontext`/`context:Context` in cell.ttl 3.11.0), `creator`'s is the union of `p:Person`/`g:Group`/`o:Organization`, `parties`'s is `cell:ACell` itself (value is the concrete subclass, not `xsd:string`; range narrowed from `cell:Cell` in 3.7.0), `templateShape`'s and `shape`'s are both `sh:NodeShape` — two separate arrows from two different boxes (`TCell` vs `ACell`) to what may be drawn as the same target, since they're two distinct properties, not one property under two names. No arrow should point from `Cell`, `TCell`, or `ACell` to any `cat:Node` box — that link is now asserted only on the category side (see Check 14).
...
**Check 13 — `topic.ttl` matches `images/topic-ontology/topic.png`**: Current as of topic.ttl 1.13.0 — verified aligned. Redrawn for the context→topic rename and the `subject` range broadening: `Topic` shows only `template`; `SCtopic` (subClassOf `Topic`) shows `subject` (targeting `xsd:anyURI` — any resource IRI, not necessarily `p:Person`/`g:Group`/`o:Organization`) and `claimant` (targeting `p:Person`/`g:Group`/`o:Organization`, not `i:PDNidentifier`) — no `about-by` arrow, matching the earlier deletion of `context:about-by`. No leaf subtype boxes below `SCtopic`, matching the earlier deletion of `SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext` — `SCtopic` has no subclasses. This diagram is the ontology-level picture of `topic:Topic`'s structure. After any change to `topic.ttl` or to this diagram, verify:

- **13a** — every property arrow shown off `Topic` in the diagram (`template`) has a corresponding `topic:` property in `topic.ttl` with `rdfs:domain topic:Topic`, and its target type matches the property's `rdfs:range`.
- **13b** — every property arrow shown off `SCtopic` in the diagram (`subject`, `claimant`) has a corresponding `topic:` property with `rdfs:domain topic:SCtopic`; `claimant`'s target in the diagram must match its actual `rdfs:range` — a union of `p:Person`/`g:Group`/`o:Organization`, not `i:PDNidentifier`; `subject`'s target must match its actual `rdfs:range` — any resource IRI (`xsd:anyURI`), not a Person/Group/Organization union. No `about-by` arrow should appear — `context:about-by` was deleted (context.ttl 1.9.0).
- **13c** — every `topic:` property with domain `topic:Topic` or `topic:SCtopic` defined in `topic.ttl` appears in the diagram under the correct box (catches new properties added to the ttl but never drawn, or drawn under the wrong box).
- **13d** — no subclasses appear below `SCtopic` — `topic.ttl` defines none (`SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext` were deleted in context.ttl 1.8.0). If any reappear here or in `topic.ttl`, reconcile them.
