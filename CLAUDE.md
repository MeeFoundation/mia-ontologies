...
**Check 13 — `cell.ttl` matches `images/cell-ontology/cell.png`**: ⚠️ **Known stale as of cell.ttl 3.4.0** — the diagram almost certainly still shows the four retired properties `sbs` (off `Cell`) and `obs`/`sbo`/`obo` (off `MultiParty`), replaced by a single `cell:sc-context` (domain `cell:Cell`, range `context:SCcontext`, see context.ttl 1.7.0). `MultiParty` should now show no arrows of its own at all — it exists purely for party-count classification. The earlier catNode staleness noted here is resolved (moot, since `cell:catNode` has been removed entirely — no arrow from `Cell` to a `Node` box is correct). This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split — it now shows only the content facet; the tree-hierarchy side it used to show moved to `category.ttl`'s diagram, Check 15) — the party-composition hierarchy and its content-linking properties. Unlike Check 11 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask; here, though, the resolution is known (the diagram needs `sbs`/`obs`/`sbo`/`obo` replaced by one `sc-context` arrow off `Cell`) and is simply waiting on a redraw. After any change to `cell.ttl`'s `cell:Cell` section or to this diagram, verify:
...
**Check 14 — `context.ttl` matches `images/context-ontology/context.png`**: ⚠️ **Known stale as of context.ttl 1.7.0** — three discrepancies await a redraw: (1) the diagram still shows a `cell` property arrow off `Context`, but `context:cell` was removed (it duplicated, in the reverse direction, the link already carried by `cell:sc-context`/`graph`; a context's containing cell is now found only by reverse lookup — see Check 3); (2) the diagram shows `subject`/`claimant` targeting `i:PDNidentifier`, but their range was relaxed to a union of `p:Person`/`g:Group`/`o:Organization` (see `persona-shacl.ttl`'s `:SCcontextShape`); (3) the intermediate class box is still labeled `XBXcontext`, but it was renamed `context:SCcontext` (Subject-Claimant context) — the four leaf subtypes (`SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext`) keep their names. This diagram is the ontology-level picture of `context:Context`'s structure — its (now sole) `template` property, the intermediate `context:SCcontext` class (carrying `about-by`/`subject`/`claimant`), and the four classified subtypes (`SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext`) below it. Like Check 13, this does not presume which side is authoritative when the two disagree — surface the discrepancy and ask; here, though, the resolution is known (the diagram needs the `cell` arrow removed, the `subject`/`claimant` target updated, and the `XBXcontext` box renamed `SCcontext`) and is simply waiting on a redraw. After any change to `context.ttl` or to this diagram, verify:
...
- **14d** — the class hierarchy shown (`Context` → `SCcontext` → `SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext`) matches `context.ttl`'s actual `rdfs:subClassOf` relationships.
