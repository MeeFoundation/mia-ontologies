...
- **Cell ontology** (`cell.ttl`): defines `cell:Cell` — the self-contained *content* facet of a cell, carrying only what's common to every cell (`folder` — carried `note` too until cell.ttl 3.15.0 merged the separate notes/files folder hierarchies into one; cell.ttl 3.16.0 documented the folder ownership boundary rule — a subfolder belongs to a different, nested cell, not this one, iff it directly contains a `category-databook`-typed DataBook file, checkable one folder at a time with no category-tree traversal), plus two further orthogonal facets: `cell:TCell` (template content, carrying `templateShape`) and `cell:ACell` (actual/instantiated content, carrying `parties`, `creator`, `subject`/`partyTopics`/`otherTopics`, and `shape` — its party composition is the `OneParty`/`MultiParty`(abstract)/`TwoParty`/`ThreePlusParty` hierarchy, hanging off `ACell` rather than `Cell` directly). `subject` (an `owl:AnnotationProperty`, range `xsd:anyURI`) identifies the one or two resource(s) — e.g. a `p:Person`/`g:Group`/`o:Organization` — the cell's relationship is about; `partyTopics` (an `owl:ObjectProperty`, range `topic:SCTopicGraph`) links to the required baseline of topic DataBooks backing the cell's content, one or more per party required, cardinality enforced per party count; `otherTopics` (same range) links to any number of additional topics beyond that baseline, optional and unbounded regardless of party count (cell.ttl 3.14.0 split of the former single `topics` property). A cell needing both facets (e.g. a template cell that also carries real party data) is simply multi-typed with both. A `cell:Cell` carries no tree position of its own, and isn't typed `cell:ACell` (so carries none of the above) until it has real content.
...
| `cell-shacl.ttl` | SHACL validation shapes for `cell:Cell` DataBook instances, split across shapes matching cell.ttl 3.7.0's facet split: `:CellShape` (target `cell:Cell`) — `folder` cardinality (dropped its `note` cardinality check in cell-shacl.ttl 3.13.0, alongside cell.ttl 3.15.0's removal of `cell:note`); `:TCellShape` (target `cell:TCell`) — `templateShape` cardinality (at most one; deliberately not constrained to `sh:class sh:NodeShape` since its value is only typed as such in `cell-templates-shacl.ttl`, which Tier 1 excludes from its merged-data run); `:ACellShape` (target `cell:ACell`) — `parties` required and constrained to be the class `cell:OneParty`/`cell:TwoParty`/`cell:ThreePlusParty`, `subject` values each constrained via `sh:nodeKind sh:IRI` (not `sh:class`, since its range is `xsd:anyURI` not a topic class; cardinality no longer set here), `otherTopics` values (if any) constrained to be a `topic:SCTopicGraph` (uniformly optional/unbounded regardless of party count), `creator` (if present) constrained to be a `p:Person`, `g:Group`, or `o:Organization`, and `shape` cardinality (at most one, same reasoning as `templateShape`); plus three new per-party shapes (cell.ttl/cell-shacl.ttl 3.14.0/3.12.0) — `:OnePartyShape`/`:TwoPartyShape`/`:ThreePlusPartyShape` (target `cell:OneParty`/`cell:TwoParty`/`cell:ThreePlusParty` directly) — enforcing `subject` as exactly 1/1..2/exactly 1 and `partyTopics` (each constrained to `topic:SCTopicGraph`) as exactly 1/2..4/at least 3 respectively, replacing the old single `topics` property's uniform "at least one, no upper bound" rule |
...
| `example/topics/self.self(ownership)(ownership)(22).databook.md` | Alice's possessions — wallet, health insurance card, SSN card |
...
| `example/topics/self.self(passport)(passport)(19).databook.md` | Alice's US passport — legal name, DOB, passport#, issue/expiry, place of birth, gender marker, photo |
| `example/topics/self.self(health-wellness)(health-wellness)(17).databook.md` | Alice's physical characteristics — height, eye color, hair color |
...
Triplestore (Fuseki) — loads all DataBook files directly:
  ...
  ├─ example/topics/self.self(health-wellness)(health-wellness)(17).databook.md
...
3. **Application Ontologies** (peer, not nested):
   ...
   - `cell.ttl`: defines `cell:Cell`, the content facet of a cell (topic links common to all cells), split into `cell:TCell` (template facet) and `cell:ACell` (actual facet, carrying party composition); mutually imports `topic.ttl`
...
**Check 2 — Filename convention**: Every topic filename must follow `<subject>.<claimant>(<containing-cell>)(<NN>).databook.md`. `<subject>` must be `self` when the subject is `:Self`, or the full hyphenated lowercase name otherwise. `<claimant>` must be `self` when the claimant is `:Self`, or the full hyphenated lowercase name otherwise — except for `c:Group` topics, where it must be the literal string `members`. `(<containing-cell>)` encodes the local name of the one `cell:Cell` DataBook whose `partyTopics` or `otherTopics` field references this topic (found by reverse lookup — a topic carries no field pointing back at its cell), always as exactly two segments: the cell's own local name, then its own `catType` kebab-cased, e.g. `(bob-johnson)(others)`, `(ownership)(ownership)` (see [Category/Cell DataBook Filename Convention](#categorycell-databook-filename-convention)). `(<NN>)` is the zero-padded two-digit topic number. If a filename does not match this pattern, rename it to conform.

**Check 3 — containing-cell ↔ filename consistency**: For every topic DataBook in `example/topics/` (excluding `under-development/`), find the one cell DataBook (in `example/Cells/`, the only remaining per-node instance tree — canonical templates have no cells of their own since category.ttl 1.8.0) whose `partyTopics` or `otherTopics` field references this topic's `id` — a topic carries no field pointing back at its cell, so this is always a reverse lookup, never a direct read. That cell's filename, **minus a trailing `-cell` suffix**, must equal the `(<containing-cell>)` segment extracted from the topic's filename. The topic filename always uses two separate parenthetical segments before the number (e.g. `(bob-johnson)(others)`) — concatenate them as `bob-johnson(others)` to form the expected local name. A topic that resolves to zero or more than one referencing cell is also an error. Run:
...
link_fields = ('partyTopics', 'otherTopics')
...
If mismatches appear, the filename `(<containing-cell>)` segments are authoritative — update the cell DataBook's `partyTopics`/`otherTopics` value to reference the correct topic (or rename the topic file if it was misnamed).
...
- **10f — Cell box border style matches `mia.parties`**: Per the Key legend, a cell box is drawn with one of three border styles — a single border ("Cell"), a double border ("Two-Party Cell"), or a bold/double border ("Multi-Party Cell") — corresponding to `cell:OneParty`, `cell:TwoParty`, and `cell:ThreePlusParty` respectively. The border style shown for a cell box must match the actual `mia.parties` value of the cell DataBook associated with that box's category (via `mia.cell`). This is a visual check (no script) — e.g. `people2.png`'s "Dr. Jane" box is drawn with a single border ("Cell"), which must match `jane-kolpakova-cell.databook.md`'s `mia.parties: "cell:OneParty"`.
...
**Check 12 — `cell.ttl` matches `images/cell-ontology/cell.png`**: Current as of cell.ttl 3.15.0 — verified aligned. The diagram no longer shows a `note` arrow off `Cell`, matching `cell.ttl` 3.15.0's removal of `cell:note` (the separate notes/files folder hierarchies merged into one single hierarchy under `cell:folder`); also correctly shows the `partyTopics`/`otherTopics` split (cell.ttl 3.14.0) and the `t:SCTopicGraph` label (topic.ttl 1.15.0), both drawn as two separate arrows off `ACell` to a `t:SCTopicGraph` box. `Cell` (abstract, blue) carries only what's common to every cell regardless of facet: `folder`. `Cell` splits into two orthogonal facets, mirroring `category.ttl`'s Category/Node split (see Check 14): `TCell` (abstract, blue, the template facet) carries `templateShape` (to a `sh:NodeShape` box); `ACell` (abstract, blue, the actual/instantiated facet) carries `subject` (to an `xsd:anyURI` box, 1..2 — renamed from `primary` in cell.ttl 3.12.0, range widened from `topic:SCTopicGraph`), `partyTopics` (to a `t:SCTopicGraph` box, cardinality varying by party count — split from `topics`/`secondary`, cell.ttl 3.14.0), `otherTopics` (to a `t:SCTopicGraph` box, 0..N uniformly — the other half of that same split), `shape` (to a `sh:NodeShape` box), `creator` (to a union of `p:Person`/`o:Organization`/`g:Group`), and `parties` (to `OneParty`/`TwoParty`/`ThreePlusParty` 1..1). The party-composition hierarchy — `OneParty` and `MultiParty` (abstract) → `TwoParty`/`ThreePlusParty` — hangs off `ACell`, not `Cell` directly; `TCell` has no subclasses of its own, since a template cell individual is instead multi-typed with both `TCell` and its `ACell`-lineage class (e.g. `OneParty`) — see `cell-templates.ttl`. `MultiParty` shows no arrows of its own, matching `cell:subject`/`cell:partyTopics`/`cell:otherTopics`'s domain being the broader `cell:ACell` rather than `cell:MultiParty`. No arrow points from `Cell`, `TCell`, or `ACell` to any `cat:Node` box, matching that link now being asserted only on the category side (see Check 14). This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split, again for the 3.7.0 TCell/ACell split, again for the 3.10.0 graph/sc-context → primary/secondary rename, again for the 3.12.0 primary/secondary → subject/topics rename, again for the topic.ttl 1.15.0 SCtopic → SCTopicGraph label fix and the cell.ttl 3.14.0 topics → partyTopics/otherTopics arrow split, and most recently for the cell.ttl 3.15.0 removal of the `note` arrow) — the party-composition hierarchy and its content-linking properties. Unlike Check 10 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask. After any change to `cell.ttl` or to this diagram, verify:

- **12a** — every property arrow shown off `Cell` (`folder`) has a corresponding `cell:` property in `cell.ttl` with `rdfs:domain cell:Cell`. Every arrow off `TCell` (`templateShape`) has `rdfs:domain cell:TCell`; every arrow off `ACell` (`shape`, `creator`, `parties`, `subject`, `partyTopics`, `otherTopics`) has `rdfs:domain cell:ACell` — `creator` and `parties` were narrowed here from `cell:Cell` in cell.ttl 3.7.0, `subject`/`topics` (renamed from `primary`/`secondary`, themselves renamed from `graph`/`sc-context`) in 3.10.0/3.12.0, and `topics` itself split into `partyTopics`/`otherTopics` in 3.14.0. `MultiParty` should show no arrows of its own — it carries no property, since `cell:subject`/`cell:partyTopics`/`cell:otherTopics`'s domain is the broader `cell:ACell`. Each arrow's target type in the diagram must match the property's `rdfs:range` — `subject`'s is `xsd:anyURI` (widened from `topic:SCTopicGraph` in cell.ttl 3.12.0, since its value is the resource(s) the cell is about, not a topic container; `subject` is an `owl:AnnotationProperty`, unlike the other `ACell` arrows, but the diagram doesn't visually distinguish annotation from object properties by arrow style anywhere — `folder` off `Cell` is an annotation property drawn identically to `templateShape`, an object property — so `subject` is correctly drawn the same way, not differently), `partyTopics`'s and `otherTopics`'s are both `topic:SCTopicGraph` (unchanged range from `topics`; narrowed from plain `topic:TopicGraph` for `primary`/`graph` in 3.10.0; renamed from `context:SCcontext`/`context:Context` in cell.ttl 3.11.0; the diagram's box for this target still reads the retired `t:SCtopic` label — see the staleness note above), `creator`'s is the union of `p:Person`/`g:Group`/`o:Organization`, `parties`'s is `cell:ACell` itself (value is the concrete subclass, not `xsd:string`; range narrowed from `cell:Cell` in 3.7.0), `templateShape`'s and `shape`'s are both `sh:NodeShape` — two separate arrows from two different boxes (`TCell` vs `ACell`) to what may be drawn as the same target, since they're two distinct properties, not one property under two names. No arrow should point from `Cell`, `TCell`, or `ACell` to any `cat:Node` box — that link is now asserted only on the category side (see Check 14). No `note` arrow should appear off `Cell` at all — `cell:note` was removed in cell.ttl 3.15.0.
...
**Check 15 — `images/cat-cell-topic.png` matches example usage**: current as of cell.ttl 3.15.0 (corrected — this check previously described the diagram in a stale pre-3.12.0 form that no longer matches what's actually drawn; the diagram itself had already been redrawn to the current `c:partyTopics`/`c:otherTopics`/`c:subject` model before that mismatch was caught). Redrawn for the context→topic rename (renamed from `images/cat-cell-context.png`). The Cell key's blue **Subject** legend entry corresponds to the blue text shown at the top of each cell box (e.g. "Self", "Self, Bob", "BHS") — this is exactly that box's cell's `c:subject` value(s), by name, not a circle. The **Topic = Party** legend entry is a thick, single-ring circle: a `topic:SCTopicGraph` linked via that cell's `c:partyTopics` (the required per-party baseline). The **Other Topic** legend entry is a double-ring circle: a `topic:SCTopicGraph` linked via `c:otherTopics` (any number of additional topics beyond that baseline). This diagram illustrates representative cell/category associations, generically rather than tied to a specific example instance: "Work" (a `cat:CategoryDefined` representing `cat:Work`, no override label), "Organization / Acme" (a `cat:CategoryDefined` representing `cat:Organization`, `cat:label`-renamed "Acme"), "Favorites" (a hypothetical `cat:UserDefined` category with no canonical counterpart, not tied to any real example data — there is currently no real `cat:UserDefined` example in the tree), "Others / Bob Johnson" (a `cat:CategoryDefined` representing `cat:Others`, `cat:label`-renamed "Bob Johnson" — a `cell:TwoParty` cell, all four topic link types filled), and "Affiliations / Boston Hub Society" (a `cell:ThreePlusParty` cell with two other-party members, Carol and BHS) — it replaces the earlier `images/cell-ontology/cells+contexts.png`. Each box's header shows `cat:catType` (green) over `cat:label` (bold) when a label is set, or just `catType` alone otherwise (e.g. "Work", which has no override label). The Key legend's gray icon swatches are labeled "Shared" — these are the file-system/chat objects common to every cell; formerly three icons (`cell:folder`, `cell:note`, chat), now two (`cell:folder`, chat) since `cell:note` was removed (cell.ttl 3.15.0, merging the separate notes/files folder hierarchies into one under `cell:folder`) — the single folder icon now also represents the cell's folder note, found by naming convention inside that one folder. The Key legend's two color swatches now correctly read "Organization" and "Person," matching `category.ttl`'s `cat:Organization`/`cat:Person` (a brief `cat:Personal`/`cat:Organizational` rename attempt was tried and reverted; this diagram, `images/category-ontology/category.png` (Check 14), and every other diagram sharing this Key legend were all redrawn to match the final `cat:Person`/`cat:Organization` naming). Re-verify each box's `Topic = Party`/`Other Topic` circles and blue `Subject` text remain a valid illustration of the properties and cardinalities described in the Cell and Category Ontology sections of `README.md` after any change to those properties.
...
**Check 17 — `partyTopics` distinct-subject count matches party class**: `cell:partyTopics`'s cardinality (cell-shacl.ttl's `:OnePartyShape`/`:TwoPartyShape`/`:ThreePlusPartyShape`) guarantees *enough* topics per party count, but not that they're topics *about* the right number of distinct parties — a cell could satisfy the count while every topic repeats the same `t:subject` (e.g. two `partyTopics` both with `subject: ":Self"` on a `TwoParty` cell). The additional invariant: across all of a cell's `partyTopics` (found via the topic's own `mia.subject`, not `mia.claimant`), the number of **distinct** `t:subject` values must be at least 1 for `cell:OneParty`, 2 for `cell:TwoParty`, and 3 for `cell:ThreePlusParty` — one per party in the relationship. This is not itself an OWL/SHACL-expressible constraint (it requires dereferencing each `partyTopics` value's own `subject`, not just counting `partyTopics` values), so it's checked here instead. Run:
...
expected = {'cell:OneParty': 1, 'cell:TwoParty': 2, 'cell:ThreePlusParty': 3}
violations = 0
for f in glob.glob('example/Cells/**/*-cell.databook.md', recursive=True):
    ...
    need = expected[parties]
    if len(subs) < need:
        violations += 1
        print(f'VIOLATION {parties} distinct_subjects={len(subs)} need>={need} subs={subs} {f}')
...
In `README.md`, every mention of a class defined in `persona.ttl` must appear in backticks with the `p:` prefix (e.g. `p:Persona`, `p:Wallet`), every mention of a class or property defined in `topic.ttl` must appear in backticks with the `t:` prefix (e.g. `t:template`, `t:subject`), every mention of a class or property defined in `cell.ttl` must appear in backticks with the `c:` prefix (e.g. `c:Cell`, `c:parties`), and every mention of a class or property defined in `category.ttl` must appear in backticks with the `cat:` prefix (e.g. `cat:Category`, `cat:catType`). Every capitalized mention of `Person` (the CCO class) must also appear in backticks. These formatting rules do **not** apply inside headings or subheadings.
