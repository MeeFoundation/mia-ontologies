...
for f in glob.glob('example/Cells/**/*.databook.md', recursive=True):
    ...
    for g in as_list(mia.get('member')) + as_list(mia.get('topic')):
        ...
        for shape in templates:
            if shape == 'pshapes:JSContactCardPersonShape':
...
**Check 31 — every cell whose category's `TemplateCell` has `cell:isTopicCell true` carries a `topic` value**: `cell:isTopicCell` (`cat-templates.ttl`) says what Lazy Instantiation's clone of a category is expected to look like — `true` means a real cell of that category is expected to carry `cell:topic` content (and be typed `cell:TopicCell`), `false` means none is expected at instantiation. This check tests that expectation in one direction only: `isTopicCell true` with no `topic` value on a real cell is always a violation — either the cell is missing content its category promises, or the template is stale. The converse is **not** a violation: a user may manually add a `cell:topic` to a cell whose category's template says `false`, choosing a template for that content as they do, and the cell is then typed `cell:TopicCell` all the same (e.g. cell-12, `Sophia Walker(immediate-family).databook.md` — a `cat:ImmediateFamily` cell, `isTopicCell false`, whose Contact Info topic about Alice's daughter Sophia was added by hand because Sophia has no instance of the app and so cannot be one of the cell's members). Such a category's `TemplateCell` should still carry a `cell:topicGraphShape` naming the shape that manually-added topic conforms to, which is what keeps Check 27 satisfied — `cell:topicGraphShape` alongside `cell:isTopicCell false` is legal, and Check 32 constrains only the `true` case. This is not itself an OWL/SHACL-expressible constraint (same reasoning as Checks 18/21/23/24/25/26/27/28/29/30 — it requires cross-referencing each cell's own `category` against `cat-templates.ttl`'s `isTopicCell` value, not just parsing one file's own YAML or Turtle in isolation), so it's checked here instead. Run:
...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    # One direction only: isTopicCell true demands a topic. A manually-added
    # topic on an isTopicCell false cell is legal (see cell-12) and not flagged.
    if has_topic or not expected:
        continue
    violations += 1
    print(f"VIOLATION {path}: category cat:{cat_local} has isTopicCell=true but cell has no topic value")
print("Every isTopicCell:true cell carries a topic value." if violations == 0 else f'{violations} violation(s) found.')
```
