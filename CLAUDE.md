...
| File | Purpose |
|------|---------|
| `example/topics/paula-walker.self(paula-walker)(acme)(06).databook.md` | Paula Walker as Alice's Acme colleague — claimed by Alice |
...
| `example/topics/self.self(texas-vital-records)(state)(24).databook.md` | Alice's Texas birth certificate — legal names, maiden name |
| `example/topics/self.self(paradise)(municipality)(18).databook.md` | Alice's Paradise, CA address — current residence (2025–present) |
| `example/topics/self.self(boston)(municipality)(13).databook.md` | Alice's Boston, MA address — previous residence (2020–2025) |
| `example/topics/self.self(social-security-administration)(federal)(23).databook.md` | Alice's Social Security Number |
...
| `example/topics/self.self(paula-walker)(acme)(20).databook.md` | Alice's Acme employee topic; social network with Paula Walker |
| `example/topics/self.self(alice-walker)(acme)(10).databook.md` | Alice's business card (JSContactCard) — name, email, phone, employer, job title |
| `example/topics/self.self(california-dmv)(state)(15).databook.md` | Alice's California driver's license — legal name, DOB, DL#, expiry, photo |
| `example/topics/self.self(passport)(federal)(19).databook.md` | Alice's US passport — legal name, DOB, passport#, issue/expiry, place of birth, gender marker, photo |
...
Triplestore (Fuseki) — loads all DataBook files directly:
  ...
  ├─ example/topics/paula-walker.self(paula-walker)(acme)(06).databook.md
...
**Check 12 — `cell.ttl` matches `images/cell-ontology/cell.png`**: ⚠️ **Stale as of cell.ttl 3.15.0** — the diagram is behind in two independent ways. First, a facet-split gap: it shows a single `topics` arrow off `ACell` (to a `t:SCtopic` box, itself the retired pre-1.15.0 class name needing a fix to `t:SCTopicGraph`), but `cell.ttl` 3.14.0 split that single property into `partyTopics` (the required per-party baseline) and `otherTopics` (any number of additional topics) — the diagram needs manual redrawing to show two separate arrows off `ACell`, both targeting a `t:SCTopicGraph` box, in place of the single `topics` arrow. Second, a removed-property gap: the diagram still shows a `note` arrow off `Cell` (to an `xsd:anyURI` box), but `cell.ttl` 3.15.0 removed `cell:note` entirely, merging the separate notes/files folder hierarchies into one single hierarchy under `cell:folder` — the diagram needs manual redrawing to drop the `note` arrow, leaving only `folder` off `Cell`. Otherwise current: `Cell` (abstract, blue) carries only what's common to every cell regardless of facet: `folder`. `Cell` splits into two orthogonal facets, mirroring `category.ttl`'s Category/Node split (see Check 14): `TCell` (abstract, blue, the template facet) carries `templateShape` (to a `sh:NodeShape` box); `ACell` (abstract, blue, the actual/instantiated facet) carries `subject` (to an `xsd:anyURI` box, 1..2 — renamed from `primary` in cell.ttl 3.12.0, range widened from `topic:SCTopicGraph`), `partyTopics` (to a `t:SCTopicGraph` box, cardinality varying by party count — split from `topics`/`secondary`, cell.ttl 3.14.0), `otherTopics` (to a `t:SCTopicGraph` box, 0..N uniformly — the other half of that same split), `shape` (to a `sh:NodeShape` box), `creator` (to a union of `p:Person`/`o:Organization`/`g:Group`), and `parties` (to `OneParty`/`TwoParty`/`ThreePlusParty` 1..1). The party-composition hierarchy — `OneParty` and `MultiParty` (abstract) → `TwoParty`/`ThreePlusParty` — hangs off `ACell`, not `Cell` directly; `TCell` has no subclasses of its own, since a template cell individual is instead multi-typed with both `TCell` and its `ACell`-lineage class (e.g. `OneParty`) — see `cell-templates.ttl`. `MultiParty` shows no arrows of its own, matching `cell:subject`/`cell:partyTopics`/`cell:otherTopics`'s domain being the broader `cell:ACell` rather than `cell:MultiParty`. No arrow points from `Cell`, `TCell`, or `ACell` to any `cat:Node` box, matching that link now being asserted only on the category side (see Check 14). This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split, again for the 3.7.0 TCell/ACell split, again for the 3.10.0 graph/sc-context → primary/secondary rename, again for the 3.12.0 primary/secondary → subject/topics rename, and now due for a topic.ttl 1.15.0 SCtopic → SCTopicGraph label fix, a cell.ttl 3.14.0 topics → partyTopics/otherTopics arrow split, and a cell.ttl 3.15.0 removal of the `note` arrow) — the party-composition hierarchy and its content-linking properties. Unlike Check 10 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask. After any change to `cell.ttl` or to this diagram, verify:
