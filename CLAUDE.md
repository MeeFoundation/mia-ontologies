...
**Check 13 — `cell.ttl` matches `images/cell-ontology/cell.png`**: The staleness previously flagged here (a missing `catNode` arrow) is now moot — `cell:catNode` has been removed from `cell.ttl` entirely (see category.ttl 1.6.0's reinstated `cat:cell`, asserted on the node instead), so a diagram with no arrow from `Cell` to a `Node` box is now correct rather than stale. This has not been independently re-verified against the current diagram pixels this round, though — treat as unconfirmed rather than known-good until reviewed. This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split — it now shows only the content facet; the tree-hierarchy side it used to show moved to `category.ttl`'s diagram, Check 15) — the party-composition hierarchy and its content-linking properties. Unlike Check 11 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask, since fixing it might mean updating `cell.ttl` (e.g. adding a missing property) or might mean the diagram is simply stale and needs redrawing. After any change to `cell.ttl`'s `cell:Cell` section or to this diagram, verify:

- **13a** — every property arrow shown off `Cell` (`num-parties`, `sbs`, `graph`, `note`, `folder`) has a corresponding `cell:` property in `cell.ttl` with `rdfs:domain cell:Cell`; every arrow shown off `MultiParty` (`obs`, `sbo`, `obo`) has `rdfs:domain cell:MultiParty`. Each arrow's target type in the diagram must match the property's `rdfs:range`. No arrow should point from `Cell` to any `cat:Node` box — that link is now asserted only on the category side (see Check 15).
...
**Check 14 — `context.ttl` matches `images/context-ontology/context.png`**: ⚠️ **Known stale as of context.ttl 1.6.0** — two discrepancies await a redraw: (1) the diagram still shows a `cell` property arrow off `Context`, but `context:cell` was removed (it duplicated, in the reverse direction, the link already carried by `cell:sbs`/`obs`/`sbo`/`obo`/`graph`; a context's containing cell is now found only by reverse lookup — see Check 3); (2) the diagram shows `subject`/`claimant` targeting `i:PDNidentifier`, but their range was relaxed to a union of `p:Person`/`g:Group`/`o:Organization` (see `persona-shacl.ttl`'s `:XBXcontextShape`). This diagram is the ontology-level picture of `context:Context`'s structure — its (now sole) `template` property, the intermediate `context:XBXcontext` class (carrying `about-by`/`subject`/`claimant`), and the four classified subtypes (`SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext`) below it. Like Check 13, this does not presume which side is authoritative when the two disagree — surface the discrepancy and ask; here, though, the resolution is known (the diagram needs the `cell` arrow removed and the `subject`/`claimant` target updated) and is simply waiting on a redraw. After any change to `context.ttl` or to this diagram, verify:
...
- **14b** — every property arrow shown off `XBXcontext` in the diagram (`about-by`, `subject`, `claimant`) has a corresponding `context:` property with `rdfs:domain context:XBXcontext`; `subject`'s and `claimant`'s target in the diagram must match their actual `rdfs:range` — a union of `p:Person`/`g:Group`/`o:Organization`, not `i:PDNidentifier`.
...
**Check 15 — `category.ttl` matches `images/category-ontology/category.png`**: ⚠️ **Known stale as of category.ttl 1.6.0** — the diagram was last verified aligned showing *no* arrow from any `Node` subclass to a `Cell` box (when `cell:catNode`, on the cell side, was the sole cell/category link). That link has now reversed: `cat:cell` (domain `cat:Node`, spanning `Canonical`/`Copy`/`UserDefined` alike) is the sole link, asserted on the node instead — so the diagram now needs a `cell` arrow added from `Node` (or from each of its three subclasses) to a `Cell` box. This diagram is the ontology-level picture of `cat:Category`'s and `cat:Node`'s structure: `cat:Category` (abstract, blue) carries only `catType`; `cat:Person`/`cat:Organization` (abstract, blue) are its direct subclasses, each with representative leaf examples (Affiliations/People/Work under Person; Suppliers/Employees under Organization); separately, `cat:Node` (abstract, blue) carries `child` (self-loop) and now `cell` (to a `Cell` box), and splits into `cat:Canonical`/`cat:Copy`/`cat:UserDefined` (all concrete, black) — `Canonical` has the `category` arrow to `Category` (recording which class a canonical node represents), `Copy` has `copiedFrom` back to `Canonical`, and both `Copy` and `UserDefined` have a `label` arrow (the shared union-domain property). This diagram does not presume which side is authoritative when the two disagree — surface the discrepancy and ask; here, though, the resolution is known (the diagram needs the `cell` arrow added) and is simply waiting on a redraw. After any change to `category.ttl` or to this diagram, verify:
