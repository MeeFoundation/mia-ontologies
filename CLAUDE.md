...
**Check 15 — `images/cat-cell-topic.png` matches example usage**: Current as of the 8-aug-2026 redraw, which recolored several boxes to illustrate `UserDefined` folders more prominently. The Key legend is unchanged and still split into two separate boxes on the right: a **Category** legend (title "Folder name") giving the three folder-color swatches — Person (tan), UserDefined (purple), Organization (light blue). These three names are *not* OWL classes — category.ttl 1.31.0 deleted `cat:Folder` and its `cat:CategoryDefined`/`cat:UserDefined` subclasses outright — they're a purely visual gloss on README's "Category Folders" section rule: tan/light-blue folders are category-defined (their cell's `c:origin` names the `cat:Person`- or `cat:Organization`-side subclass it was instantiated from), purple folders are user-defined (no `c:origin` value at all). The **Cell** legend gives: a blue **Subject** heading over a green-filled circle labeled "Claimed by Other", a white/outlined circle labeled "Claimed by Self", and a gray swatch labeled "Shared"; two more circle entries labeled "Member Topic" and "Other Topic" (mapping to `c:memberTopics`/`c:otherTopics` respectively — visually similar at normal viewing size, so lean on the legend's text labels rather than trying to eyeball a ring-style difference); and three cell-box border-style entries — "Multi-Member Cell" (bold/double border), "Two-Member Cell" (double border), "Single-Member Cell" (single border) — these are exactly the `cell:label` strings cell.ttl 3.17.0 updated `cell:ThreePlusMember`/`cell:TwoMember`/`cell:OneMember` to (see Check 12d). Every topic circle in the diagram body carries an explicit subject-name label (e.g. "Bob", "Self", "BHS") baked directly into the circle, not just a bare ring. This diagram illustrates representative cell/category associations, generically rather than tied to a specific example instance — five boxes, each with a single-line green folder-name header (no separate `catType`/`label` split): `Favorites` (purple/`UserDefined`, `cell:OneMember`, subject "Self", one white "Self" topic), `Employees` (light-blue/`Organization`, `cell:OneMember`, subject "Self", one white "Self" topic), `People` (tan/`Person`, `cell:OneMember`, subject "Self", one white "Self" topic), `Bob Johnson` (purple/`UserDefined`, `cell:TwoMember`, subject "Self, Bob", four topics — two white/self-claimed, two green/other-claimed, all four topic link types filled), and `BHS` (purple/`UserDefined`, `cell:ThreePlusMember`, subject "BHS", three topics: BHS's own profile (green), Self's BHS profile (white), and Bob's BHS member profile (green)) — it replaces the earlier `images/cell-ontology/cells+contexts.png`. Each cell box shows only a single plain folder icon (no separate "chat" icon — the two-icon pairing an earlier revision of this diagram drew is gone; see Check 12's still-open `cell:chat` discrepancy, which concerns `cell.png` only, not this diagram). Re-verify each box's `Member Topic`/`Other Topic` circles and blue `Subject` text remain a valid illustration of the properties and cardinalities described in the Cell and Category Ontology sections of `README.md` after any change to those properties.
