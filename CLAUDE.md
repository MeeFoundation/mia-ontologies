...
- **10g — Blue "Subject" annotation matches the cell's derived subject**: Per the Key legend's "Subject" entry, a cell box carries a blue "Subject" text annotation listing the name(s) of the resource(s) the cell's relationship is about, comma-separated when there is more than one. There is no stored `mia.subject`/`cell:subject` to compare against — this text must instead match the *derived* subject, computed from the co-located cell-databook's own `mia.memberTopics`/`mia.otherTopics`: if the cell has a `mia.otherTopics` entry, that topic's own `subject` is the answer; otherwise the answer is every distinct `subject` among `mia.memberTopics` (the cell's active members). This is a visual check (no script) — e.g. `finances.png`'s "Banking & Payments Firms / Citibank" box's Subject annotation reads "Self, Citibank", matching `Citibank(banking-payments).databook.md`'s two `mia.memberTopics` (topics 09 and 27), whose distinct subjects are `:Self` and `:Citibank`; `affiliations.png`'s "Boston Hub Society" box (`cell:ThreePlusMember`, no `mia.otherTopics`, three distinct `mia.memberTopics` subjects `:BHS`/`:Bob_Johnson`/`:Self`) must read "BHS, Bob, Self" — **currently stale and needs manual redraw**, since it still shows "BHS" alone from before `cell:subject` was removed.
