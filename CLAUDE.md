...
| `cell.ttl` | Cell ontology — `cell:Cell` (formerly `cell:Parties`), the content facet of a cell: the `Cell`/`MultiParty`(abstract)/`OneParty`/`TwoParty`/`ThreePlusParty` party-count hierarchy, and the properties that classify/link a cell's content (`parties`, `sc-context`, `graph`, `note`, `folder`, `creator`). `parties`'s range is `cell:Cell` itself — its value is the concrete subclass (e.g. `cell:OneParty`), not a string, mirroring `cat:category`'s class-value punning (category.ttl). `creator`'s range is a union of `p:Person`/`g:Group`/`o:Organization`, referenced by name without importing those ontologies (mirroring `context:subject`/`context:claimant`). Carries no link back to a node — that's asserted only on the category side, as `cat:cell`. Mutually imports `context.ttl` |
...
| `cell-shacl.ttl` | SHACL validation shapes for `cell:Cell` DataBook instances — `parties`/`sc-context`/`graph`/`note`/`folder`/`creator` cardinality, with `parties` required and constrained to be the class `cell:OneParty`/`cell:TwoParty`/`cell:ThreePlusParty`, `sc-context` values (if any) constrained to be a `context:SCcontext` (uniformly regardless of party count), and `creator` (if present) constrained to be a `p:Person`, `g:Group`, or `o:Organization` |
...
| `categories-person/Government/State/Birth Certificate/birth-certificate-cell.databook.md#shapes` | Per-template SHACL shapes for birth certificate context files, embedded (fragment `#shapes`) in the canonical cell-databook rather than a standalone file — run against the individual context file, not merged data |
| `shacl/jscontactcard-shacl.ttl` | Per-template SHACL shapes for JSContactCard context files — run against the individual context file, not merged data (JSContactCard has no canonical `categories-person` cell, so its shape stays standalone) |
| `categories-person/Government/State/Drivers License/drivers-license-cell.databook.md#shapes` | Per-template SHACL shapes for driver's license context files, embedded in the canonical cell-databook — run against the individual context file, not merged data |
| `categories-person/Government/Federal/Passport/passport-cell.databook.md#shapes` | Per-template SHACL shapes for passport context files, embedded in the canonical cell-databook — run against the individual context file, not merged data |
| `categories-person/Health & Wellness/Medical/Providers/Medical Appointment Info/medical-appointment-info-cell.databook.md#shapes` | Per-template SHACL shapes for medical appointment context files, embedded in the canonical cell-databook — run against the individual context file, not merged data |
...
shacl/jscontactcard-shacl.ttl — per-template shapes for JSContactCard files (only template with no canonical categories-person cell)
categories-person/Government/State/Birth Certificate/birth-certificate-cell.databook.md#shapes — embedded per-template shapes for birth certificate files
categories-person/Government/State/Drivers License/drivers-license-cell.databook.md#shapes — embedded per-template shapes for driver's license files
categories-person/Government/Federal/Passport/passport-cell.databook.md#shapes — embedded per-template shapes for passport files
categories-person/Health & Wellness/Medical/Providers/Medical Appointment Info/medical-appointment-info-cell.databook.md#shapes — embedded per-template shapes for medical appointment files
```
...
**Check 5 — Validation command completeness**: The `## Validation` section of `README.md` must document two tiers. Tier 1 uses four steps: (1) a `find example -name "*.databook.md"` loop using `databook extract` to extract turtle content and produce a merged turtle file of all context data (excluding `under-development/`); (2) a `riot` merge of that data with all application ontology TTL files and the foundation ontologies listed explicitly from `project_files/`; (3) a `grep -v owl:imports` on `persona-shacl.ttl` and `context-shacl.ttl` to collect shapes (the `shacl/` per-template files are excluded here — they target `persona:Person` and would fire incorrectly on all individuals when applied to merged data); (4) a `shacl validate` call. Tier 2 lists explicit per-file `databook extract` + `riot` + `shacl validate` commands for each template context file paired with its shapes source — a `databook extract` of the relevant `categories-person` canonical cell-databook (fragment `#shapes`) for BirthCertificate/DriversLicense/Passport/MedicalAppointment, or `shacl/jscontactcard-shacl.ttl` directly for JSContactCard. If the commands change, update the README to match.
...
**Check 12 — `cell.ttl` matches `images/cell-ontology/cell.png`**: Current as of cell.ttl 3.5.0 — verified aligned. All six properties (`creator`, `note`, `folder`, `graph`, `parties`, `sc-context`) are drawn off `Cell` only, with `sc-context` correctly targeting `SCcontext(s)`, `creator` correctly targeting a union of `p:Person`/`o:Organization`/`g:Group`, and `parties` correctly targeting `OneParty`/`TwoParty`/`ThreePlusParty` (the concrete subclasses themselves, not `xsd:string`) with cardinality `1..1`, matching `cell:parties`'s required, class-valued range; `MultiParty` shows no arrows of its own, matching `cell:sc-context`'s domain being the broader `cell:Cell` rather than `cell:MultiParty`. No arrow points from `Cell` to any `cat:Node` box, matching that link now being asserted only on the category side (see Check 14). This diagram is the ontology-level (not example-tree) picture of `cell:Cell`'s structure (redrawn for the 3.0.0/1.0.0 Cell/Category split — it now shows only the content facet; the tree-hierarchy side it used to show moved to `category.ttl`'s diagram, Check 14) — the party-composition hierarchy and its content-linking properties. Unlike Check 10 (example diagrams, where the diagram always wins), this check does not presume which side is authoritative when the two disagree — surface the discrepancy and ask. After any change to `cell.ttl`'s `cell:Cell` section or to this diagram, verify:

- **12a** — every property arrow shown off `Cell` (`creator`, `parties`, `sc-context`, `graph`, `note`, `folder`) has a corresponding `cell:` property in `cell.ttl` with `rdfs:domain cell:Cell` — `creator` included; it is a `cell:Cell`-only property, not usable on a context DataBook. `MultiParty` should show no arrows of its own — it carries no property, since `cell:sc-context`'s domain is the broader `cell:Cell`. Each arrow's target type in the diagram must match the property's `rdfs:range` — `sc-context`'s is `context:SCcontext`, `creator`'s is the union of `p:Person`/`g:Group`/`o:Organization`, `parties`'s is `cell:Cell` itself (value is the concrete subclass, not `xsd:string`). No arrow should point from `Cell` to any `cat:Node` box — that link is now asserted only on the category side (see Check 14).
- **12b** — every `cell:` property with `rdfs:domain cell:Cell` defined in `cell.ttl` appears as an arrow in the diagram, under the `Cell` box (catches new properties added to the ttl but never drawn, or drawn under the wrong box).
- **12c** — the class hierarchy `Cell` → `MultiParty` (abstract) → `TwoParty`/`ThreePlusParty`, plus `Cell` → `OneParty`, shown in the diagram matches `cell.ttl`'s actual `rdfs:subClassOf` relationships (by class local name, not just position).
...
**Check 16 — IRI roots: `mee.foundation/ontologies` for foundational files, `www.example.org` for example data**: Every foundational ontology and SHACL shapes file — `persona.ttl`, `context.ttl`, `cell.ttl`, `category.ttl`, `pdn-identity.ttl`, `group.ttl`, `organization.ttl`, `persona-templates.ttl`, their `*-shacl.ttl` companions, and the per-template files in `shacl/` — must declare its `owl:Ontology` IRI under `http://mee.foundation/ontologies/`. Canonical category/cell DataBooks (`categories-person/`, `categories-org/`) are foundational too, not example data, so their `id:` values must also be `http://mee.foundation/ontologies/...`. Conversely, every DataBook under `example/categories/` and `example/contexts/` (excluding `under-development/`) represents Alice's own example instance data, so its `id:` must be grounded under `http://www.example.org/` (or `https://www.example.org/`). Run:

```python
import os, re, glob

FOUNDATIONAL_TTL = [
    'persona.ttl', 'context.ttl', 'cell.ttl', 'category.ttl',
...
check_id_root('categories-person/**/*.databook.md', ['http://mee.foundation/ontologies/'], True)
check_id_root('categories-org/**/*.databook.md', ['http://mee.foundation/ontologies/'], True)
