...
| Topic 37 — `example/Cells/Pets/Ginger/Ginger(pets).databook.md` | Alice's basic claim identifying Ginger — backs the Ginger cell's `subject: ":Ginger"` with a real topic (see Check 22), linked as an `otherTopic` |
...
`topic.ttl`, `cell.ttl`, and `category.ttl` no longer carry `owl:versionInfo` at all — it was deleted outright (property and value) since these three files are evolving too rapidly right now for a change-log annotation to be worth maintaining. Do not re-add it to any of the three. They still carry `owl:versionIRI` and `dc:date`; bump those on a real change if convenient, but there's no change-history text to update alongside them any more. `organization.ttl`, `pdn-identity.ttl`, `cell-templates.ttl`, and `persona-templates.ttl` are unaffected by this — they still carry `owl:versionInfo` and should keep it updated on a real change, the same way `persona.ttl` does.

**`owl:versionInfo` describes only the current version — no embedded history, ever** (as of 2026-08-23): every file's `owl:versionInfo` value used to grow a nested parenthetical chain of prior versions' descriptions (`"Version 4.6.0 - ... (4.5.0 - ... (4.4.0 - ...))"@en`) — that entire pattern was removed outright from `persona.ttl`, `organization.ttl`, `pdn-identity.ttl`, `cell-templates.ttl`, and `persona-templates.ttl`, leaving each file's `owl:versionInfo` as a single bare `"Version X.Y.Z - <description of this change only>"@en` string. **Never reinsert that history when bumping a version going forward** — replace the whole string with the new version and its own description; do not append the outgoing description in parentheses. Git carries the changelog now — the same reasoning that already governs `rdfs:comment`/block-comment text in every `.ttl` file (describe current semantics only, never narrate how it got that way) now applies to `owl:versionInfo` too.

**No `owl:versionInfo` in any `*-shacl.ttl` file, ever** (as of 2026-08-23): `cell-shacl.ttl`, `topic-shacl.ttl`, `persona-shacl.ttl`, `organization-shacl.ttl`, `pdn-identity-shacl.ttl`, `cell-templates-shacl.ttl`, and `shacl/jscontactcard-shacl.ttl` all had their `owl:versionInfo` changelog removed outright, mirroring `topic.ttl`/`cell.ttl`/`category.ttl`'s existing no-versionInfo convention — this now applies uniformly to every SHACL shapes file regardless of whether its paired main ontology file still carries `owl:versionInfo` (`persona-shacl.ttl` no longer does, even though `persona.ttl` itself still does). Every one of these files still carries `owl:versionIRI` and `dc:date`; bump those on a real change, but do not re-add `owl:versionInfo` to any of them — their `rdfs:comment`s describe only current semantics, same as the main ontology files.
...
**One-time exception**: on 2026-08-13 the entire `example/Cells/` tree was renumbered once — the 19 content cells (the ones with real `mia.memberCount`/topics, previously scattered up to `cell-36`) were compacted to `cell-01`–`cell-19`, and the 17 pure scaffolding/tree-node cells (folders like `Work`, `People`, `Government` with no member content at the time) were moved to `cell-20`–`cell-36`, so the two ranges can never collide. This was safe only because the example tree is fictional data with no real external PDN peer who could hold a stale reference to the old numbers — it does not relax the "never renumber" rule above for any new or real cell going forward. (On 2026-08-23, once the "bare tree-position placeholder" exemption was removed from `cell.ttl`/`cell-shacl.ttl`, these 17 — plus `cell-37`/`cell-38`, added later for the `Pets` subtree — were each given a minimal stub `memberTopics` entry per Check 21, without renumbering; every cell in `example/Cells/` now carries real member content.)
...
- **10g — Blue "Subject" annotation matches `cell:subject`**: Per the Key legend's "Subject" entry, a cell box carries a blue "Subject" text annotation listing the name(s) of the resource(s) the cell's relationship is about, comma-separated when there are two. This text must list exactly the same value(s) — by name, not IRI — as the actual `mia.subject` value(s) of the cell-databook co-located in that box's own folder, no more and no fewer. This is a visual check (no script) — e.g. `finances.png`'s "Banking & Payments Firms / Citibank" box's Subject annotation reads "Self, Citibank", matching `Citibank(banking-payments).databook.md`'s `mia.subject: [":Self", ":Citibank"]`.
...
- **10i — Black curly-brace `{NN}` label matches the cell's own `cell-<NN>` id**: As the last line of a cell box's content (immediately below the Subject/Origin text), a cell box may carry a small black `{NN}` label in curly braces — the cell's own number. As of the 8-aug-2026 rollout this label has been added to every cell box across all 11 diagrams (a bare scaffolding folder with no rendered content box, e.g. `Work`/`Acme`/`Employees`, still carries none — deliberately: even though cell.ttl 3.31.0/cell-shacl.ttl 3.22.0 now require every one of these scaffolding cells to carry real (stub) member content, that stub is intentionally not visualized as a content box, so the box still shows no border style, Subject text, `{NN}`, or topic circles — a visual simplification, not a sign the cell lacks content); `pets.png` was drawn 22-aug-2026 and confirmed showing its "Ginger" box with `{41}` and its "Medications" box with `{40}`; all 11 diagrams were redrawn again 23-aug-2026 once every cell-databook's headers became complete, confirming this same scaffolding-cells-stay-unboxed convention holds throughout. Wherever `{NN}` appears, it must equal the zero-padded two-digit `<NN>` from the co-located cell-databook's own `id: http://www.example.org/mia/cells/cell-<NN>` (see Check 9). Don't confuse this with the numbered topic circles' `[NN]` labels (Check 10d) — those are topic numbers in square brackets attached to a circle; this is the cell's own number in curly braces attached to the box itself. This is a visual check (no automated OCR), but the script below prints every folder's actual `cell-<NN>` for quick cross-reference against whichever diagram is being checked — e.g. `people.png`'s "Bob Johnson" box shows `{16}` and its "Fred Flintstone" box shows `{17}`; `people2.png`'s "Paula Walker" (Immediate Family) box shows `{12}`, "Health & Wellness" shows `{13}`, "Jane Starostina" shows `{14}`, and "Med. App. Info" shows `{15}`; `companies.png`'s "Google"/"ATT" boxes show `{3}`/`{2}`; `finances.png`'s "Citibank" box shows `{4}`; `gov-state.png`'s "Texas Vital Records"/"California DMV" boxes show `{10}`/`{9}`; `gov-federal.png`'s "Social Security Administration"/"Department of State" boxes show `{6}`/`{5}`; `gov-municipality.png`'s "Boston"/"Paradise" boxes show `{7}`/`{8}`; `misc.png`'s "Ownership" box shows `{11}`; `affiliations.png`'s "Boston Hub Society" box shows `{1}`; `work.png`'s "Paula"/"Alice Walker" boxes show `{19}`/`{18}` — all confirmed matching their cell-databooks' actual ids. Run:
...
- **10j — Fill color and folder-name-text color match real data**: Every cell across all 11 diagrams carries exactly two independent, mechanically-checkable colors, matching Check 10a's Person/Organization/Custom fill-swatch legend (see Check 15's identical rule for `cat-cell-topic.png`): a **fill** color, applied to the folder icon itself (the cell's own content box is always plain white and never carries fill) — tan if the folder's `mia.origin` resolves to `cat:Person`, light blue if `cat:Organization`, purple/Custom if the cell has no origin at all — and a folder-**name-text** color (green/"Predefined" if the folder's `title:` equals the origin class's own `rdfs:label` verbatim, plain black/"User-defined" otherwise — always black for a no-origin/Custom cell). This is a visual check (no automated pixel/OCR comparison) — use Check 20's script (identical rule, just applied to a different set of diagrams) to compute the correct fill/text color for every real folder and cross-reference against whichever diagram is being checked. Confirmed against all 11 diagrams — e.g. `people.png`: "People"/"Others" tan fill + green text; "Bob Johnson"/"Fred Flintstone" tan fill + black text; `work.png`: "Paula"/"Alice Walker" light-blue fill + black text; `people2.png`: "Paula Walker"/"Jane Starostina"/"Med. App. Info" tan fill + black text; `companies.png`/`finances.png`/`gov-state.png`/`gov-federal.png`/`gov-municipality.png`/`misc.png`/`affiliations.png` all correctly updated too; `pets.png` (drawn 22-aug-2026, alongside `cat:Pets`/`cat:PetsHealth`/`cat:PetsMedications`): "Pets"/"Health"/"Medications" tan fill + green text (folder name matches origin label), "Ginger" tan fill + black text (origin `cat:Pets` label "Pets" ≠ folder name "Ginger"). No real example cell currently uses the Custom (no-origin, `(custom)` filename) case — all 40 currently have an origin — so no example diagram box is expected to show purple fill yet.

The 11 diagrams are: `example/images/people.png`, `example/images/people2.png`, `example/images/work.png`, `example/images/companies.png`, `example/images/finances.png`, `example/images/gov-state.png`, `example/images/gov-federal.png`, `example/images/gov-municipality.png`, `example/images/misc.png`, `example/images/affiliations.png`, `example/images/pets.png`. (`health.png` was intentionally deleted — it wasn't useful; its content, e.g. Health & Wellness/Medical/Providers, lives in `people2.png` instead. `pets.png` was drawn by the user 22-aug-2026 and confirmed against all sub-checks 10a–10j — it shows both the "Ginger" cell (cell-41, `{41}`, topics `[36]`/`[37]`) and the "Medications" cell (cell-40, `{40}`, topics `[32]`/`[33]`) as separate content boxes under the `Pets → Ginger → Health → Medications` folder nesting.)

**Check 11 — Physical folder structure IS the tree of cells in `example/Cells/`**: This check applies only to `example/Cells/` — the user's own instance tree — since there is no longer a separate canonical-instance file tree to mirror (`categories-person/`/`categories-org/` were removed in category.ttl 1.8.0). There is no longer a `mia.child`/`mia.cell` YAML list to cross-check the tree against either — category.ttl 1.31.0 deleted `cat:child`/`cat:cell` (and `cat:Folder` itself) outright, so this check has no independently-asserted list to "mirror" at all; it collapses to a pure filesystem sanity check with no YAML frontmatter parsing at all. A folder is a **cell** ("marker dir") iff it directly contains exactly one `*.databook.md` file (the only DataBook type in a user's instance tree is now cell-databook, so no `-cell` marker is needed to identify one) — that file is simultaneously the cell's real (or placeholder) content and its tree-node marker (cell.ttl's folder ownership boundary rule, 3.16.0/3.19.0). A folder can never legally hold more than one cell-databook (cell.ttl 3.22.0): a `cell:Cell` is self-contained, and letting two cells share a folder would risk a single file in that folder becoming ambiguously part of both — this check flags any such folder as an error. A plain filesystem folder with no cell-databook of its own is not a cell at all — that word stays reserved for a folder that does have one. Cell naming is not standardized — a cell's own folder name may be the category's display label, a role-based label, or anything else — but a cell-databook's own filename is now always the folder's exact verbatim name (see the Cell DataBook Filename Convention), so this check's per-folder marker test and its filename root are one and the same string. ⚠️ **Corrected, 22-aug-2026**: this used to additionally forbid bare, marker-less pass-through directories between two marker dirs — that was wrong, and directly contradicted README.md's own definition of a "regular filesystem folder" ("A folder without a matching cell DataBook is simply a regular file system folder, not a cell — **even if it contains nested cells of its own**"). A bare folder with no cell-databook of its own is legal anywhere in the tree, including between two marker dirs, as long as it isn't otherwise empty (i.e. something beneath it eventually has a cell-databook) — no current example under `example/Cells/` happens to need this (every existing folder, `example/Cells/Pets/Ginger/` included, turned out to warrant its own cell-databook after all — see `cat:Pets`'s reuse pattern below), but the rule itself remains correct and available for a future folder that's purely organizational. Run:
...
def check_tree(root):
    ...
    #  own") — so there is no ancestor-chain check here any more.
...
- **12d** — ⚠️ **Corrected, 22-aug-2026**: this item previously claimed `cell.png` displays each concrete `Cell` subtype's example `cell:label` value (`"Single-Member Cell"`, `"Two-Member Cell"`, `"Multi-Member Cell"`, later renamed `"1 Member Cell"`/`"2 Member Cell"`/`"3+ Member Cell"`) somewhere in the diagram, and that this text must match `cell.ttl`'s actual `cell:label` values. Direct inspection of the current `cell.png` shows this is not true — the diagram only draws the bare class local names (`OneMember`, `TwoMember`, `ThreePlusMember`) as box labels; no `cell:label` display string appears anywhere in it. There is accordingly nothing for this item to check in `cell.png` — the `cell:label` wording is instead verified where it's actually drawn, in the 11 example diagrams (Check 10f) and in `cat-cell-topic.png`/`images/folder-mapping.png`'s border-style legend entries (Checks 15/20). `cell:label` itself is still a real class-level default display name defined in `cell.ttl` (category.ttl 1.31.0 deleted `cat:label`, the per-instance display name it used to be distinguished from, outright, since it lived on the now-deleted `cat:Folder`) — `Cell`, `TCell`, `ACell`, and `MultiMember` are all abstract and carry no `cell:label` of their own — this item is only correcting where that text is expected to be rendered.

**Check 13 — `topic.ttl` matches `images/topic-ontology/topic.png`**: **Resolved, 22-aug-2026** — the user redrew this diagram; it now correctly shows `TopicGraph`/`SCTopicGraph` (the current topic.ttl 1.15.0 class names), not the retired pre-1.15.0 `Topic`/`SCtopic` labels. `TopicGraph` shows only `template` (targeting `p:PersonaTemplate`, matching `topic:template`'s actual `rdfs:range`); `SCTopicGraph` (subClassOf `TopicGraph`) shows `subject` (targeting `xsd:anyURI` — any resource IRI, not necessarily `p:Person`/`o:Organization`) and `claimant` (targeting `p:Person`/`o:Organization`, not `i:PDNidentifier`) — no `about-by` arrow, matching the earlier deletion of `context:about-by`. No leaf subtype boxes below `SCTopicGraph`, matching the earlier deletion of `SBScontext`/`OBScontext`/`OBOcontext`/`SBOcontext` — `SCTopicGraph` has no subclasses. This diagram is the ontology-level picture of `topic:TopicGraph`'s structure. After any change to `topic.ttl` or to this diagram, verify:
