...
| Graph 36 — `example/Cells/Pets/Ginger/Ginger(pets).databook.md` | Deliberately empty — the `cat:Pets`-category Ginger cell's required `member` entry, claimed by Alice; `member`'s requirement is about `g:subject`/`g:claimant`, not content |
...
| Graph 33 — `example/Cells/Pets/Ginger/Medical/Medical.databook.md` | Deliberately empty — one of the cell's two required `member` entries, claimed by Alice |
| Graph 57 — `example/Cells/Pets/Ginger/Medical/Medical.databook.md` | Deliberately empty — the other required `member` entry, claimed by Paula, after the cell (created by Self) was shared with Paula and became a two-member cell |
| Graph 58 — `example/Cells/Pets/Ginger/Care & Feeding/Care & Feeding.databook.md` | Deliberately empty — one of the cell's two required `member` entries, claimed by Alice |
...
| Graph 62 — `example/Cells/Things/Vehicles/RAV4/RAV4(vehicles).databook.md` | Deliberately empty — the RAV4 cell's required `member` entry, claimed by Alice |
...
**Check 18 — `topic`'s presence is what types a cell `cell:TopicCell`, and a cell's subject is derived from it plus `member`**: There is no independently-asserted `cell:subject`/`mia.subject` — who or what a cell's relationship is about is computed, not stored, by a simple two-branch rule: **if the cell has any `topic` entries** (i.e. it's typed `cell:TopicCell`), the full set of distinct `g:subject` values among them is the cell's subject (e.g. `Paula-Walker(employee).databook.md`: `member` holds Self's own graph (the member), `topic` holds Paula's graph — subject is `:Paula_Walker`; similarly `Med.-App.-Info(medical-appointment).databook.md`, a two-member cell: `member` holds Carol's and Self's graphs, `topic` holds Paula's graph — subject is `:Paula_Walker`); **otherwise** the subject is the full set of distinct `g:subject` values among `member` — the cell's own active members (e.g. `Bob-Johnson(others).databook.md`, a two-member cell with no `topic`: subject is `:Self` and `:Bob_Johnson` together; `Boston-Hub-Society(affiliations).databook.md`, a cell with three or more members with no `topic`: subject is `:BHS`, `:Bob_Johnson`, and `:Self` together). `cell:topic` is required (at least one value) but unbounded above once a cell is typed `cell:TopicCell` at all (`cell-shacl.ttl`'s `:TopicCellShape`) — every real example cell today happens to carry exactly one `topic` value, but the rule and this check both generalize to any number. One invariant follows that isn't itself OWL/SHACL-expressible (dereferencing each linked graph's own `subject`, not just counting links), so it's checked here instead: none of a cell's `topic` subjects may already be one of that cell's `member` subjects — otherwise that duplicate would silently mask a real member from the derived subject, since the presence of any `topic` value switches the derivation to the `topic` set entirely, ignoring `member` (e.g. `Jane-Starostina(primary-care-physician).databook.md`, `Health & Wellness.databook.md`, and `Medical.databook.md`/`Care & Feeding.databook.md` under `Pets/Ginger/` each keep a real `:Self`-subject graph in `member`, distinct from their own `topic` entry's subject, for exactly this reason). Run:
...
for f in glob.glob('example/Cells/**/*.databook.md', recursive=True):
    ...
    if dup:
        violations += 1
        print(f'VIOLATION {f}: topic subject(s) {sorted(s for s in dup if s)} duplicate a member subject {sorted(s for s in pt_subs if s)} — masks a real member from the derived subject')
...
If a violation is found: for a duplicated subject, either move the redundant `topic` graph's content into the matching `member` graph instead, or reconsider whether that graph really belongs in `topic` at all.
...
In `README.md`, `EXAMPLE.md`, and `APP-BEHAVIOR.md`, every mention of a class defined in `persona.ttl` must appear in backticks with the `p:` prefix (e.g. `p:Persona`, `p:Wallet`), every mention of a class or property defined in `graph.ttl` must appear in backticks with the `t:` prefix (e.g. `g:template`, `g:subject`), every mention of a class or property defined in `cell.ttl` must appear in backticks with the `c:` prefix (e.g. `c:Cell`, `c:member`), every mention of a class or property defined in `category.ttl` must appear in backticks with the `cat:` prefix (e.g. `cat:CategoryScheme`, `cat:Person`), every mention of a class or property defined in `other/pets.ttl` must appear in backticks with the `pets:` prefix (e.g. `pets:Pet`, `pets:hasSpecies`), every mention of a class or property defined in `other/vehicles.ttl` must appear in backticks with the `v:` prefix (e.g. `v:Vehicle`, `v:hasMake`) — `other/vehicles.ttl`'s own real Turtle prefix stays the verbose `vehicles:` internally (matching `agent.ttl`'s `a:`/`agent:` split below), but every doc mention uses the short `v:` alias, and every mention of a class or property defined in `agent.ttl` must appear in backticks with the `a:` prefix (e.g. `a:Agent`, `a:actsFor`) — `agent.ttl`'s own real Turtle prefix stays the verbose `agent:` internally (matching `cell.ttl`'s `c:`/`cell:` split), but every doc mention uses the short `a:` alias. Every capitalized mention of `Person` (the CCO class) must also appear in backticks. These formatting rules do **not** apply inside headings or subheadings.
