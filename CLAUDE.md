...
In `README.md`, `EXAMPLE.md`, and `APP-BEHAVIOR.md`, every mention of a class defined in `persona.ttl` must appear in backticks with the `p:` prefix (e.g. `p:Persona`, `p:Wallet`), every mention of a class or property defined in `graph.ttl` must appear in backticks with the `t:` prefix (e.g. `g:template`, `g:subject`), every mention of a class or property defined in `cell.ttl` must appear in backticks with the `c:` prefix (e.g. `c:Cell`, `c:members`), every mention of a class or property defined in `category.ttl` must appear in backticks with the `cat:` prefix (e.g. `cat:CategoryScheme`, `cat:Person`), every mention of a class or property defined in `other/pets.ttl` must appear in backticks with the `pets:` prefix (e.g. `pets:Pet`, `pets:hasSpecies`), and every mention of a class or property defined in `other/vehicles.ttl` must appear in backticks with the `vehicles:` prefix (e.g. `vehicles:Vehicle`, `vehicles:hasMake`), and every mention of a class or property defined in `agent.ttl` must appear in backticks with the `a:` prefix (e.g. `a:Agent`, `a:actsFor`) — `agent.ttl`'s own real Turtle prefix stays the verbose `agent:` internally (matching `cell.ttl`'s `c:`/`cell:` split), but every doc mention uses the short `a:` alias. Every capitalized mention of `Person` (the CCO class) must also appear in backticks. These formatting rules do **not** apply inside headings or subheadings.
