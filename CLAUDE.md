...
| Graph 57 — `example/Cells/Pets/Ginger/Medical/Medical.databook.md` | Deliberately empty — the other required `members` entry, claimed by Paula, after the cell (created by Self) was shared with Paula and became a `cell:TwoMember` cell |
...
| Graph 59 — `example/Cells/Pets/Ginger/Care & Feeding/Care & Feeding.databook.md` | Deliberately empty — the other required `members` entry, claimed by Paula, after the cell (created by Self) was shared with Paula and became a `cell:TwoMember` cell |
...
- **10f — Cell box border style no longer varies by member count (retired)**: All 12 example diagrams (the original 11 plus `travel.png`) draw a cell's own DataBook box with one uniform rounded-corner, single-border style — the Key legend's plain "Cell" swatch — regardless of `mia.memberCount`. The earlier three-way border-style convention (square corners for `cell:OneMember`, rounded single border for `cell:TwoMember`, rounded double/bold border for `cell:ThreePlusMember`) was retired project-wide; no diagram box should draw a doubled or squared border to indicate member count any more. A cell's actual member count is recorded only in its own `mia.memberCount` YAML field, not in any diagram-visible border distinction — cross-reference the co-located cell-databook's `mia.memberCount` value directly rather than the box's border style.
...
- **12d** — `cell.png` draws only the bare class local names (`OneMember`, `TwoMember`, `ThreePlusMember`, `TopicCell`) as box labels; no `cell:label` display string appears anywhere in this diagram. This is now true of every diagram: since the member-count border-style convention was retired project-wide (Check 10f), `cell:label` (`"1 Member Cell"`, `"2 Member Cell"`, `"3+ Member Cell"`) is a pure ontology-level default-display-name annotation with no current visual instantiation anywhere — not in `cell.png`, not in the 12 example diagrams, not in `cat-cell-graph.png`/`images/folder-mapping.png` (Checks 15/20). `cell:label` itself is still a real class-level annotation defined in `cell.ttl` — `Cell`, `TemplateCell`, `MemberCell`, and `MultiMember` are all abstract and carry no `cell:label` of their own; `TopicCell`, though concrete, carries no `cell:label` either, since a cell's member-count class (not `TopicCell`) is what it would have described.
...
**Check 15 — `images/cat-cell-graph.png` matches example usage**: The legend is a single box (title "Key") — there is no longer a separate "Category" legend box. It holds, in order: three fill-color swatches — Person (tan), Organization (light blue), None (Custom) (purple/lavender); a compact two-line folder-name-text formula, "Name = Category or" (blue "Name =", green "Category") / "User-defined + (Category)" (bold black) — green text means the folder's name is copied verbatim from its origin's own `rdfs:label`, black means the user gave it a different name (shown alongside its `(Category)` parenthetical), and a Custom (no-origin) folder's name is always black, never green, since it has no label to match; three claim-color swatches — a green-filled swatch labeled "Other" (claimed by someone other than the self), a gray-filled swatch labeled "Delegate" (claimed by an invited `agent:Agent` — see README.md's [Agent Ontology](README.md#agent-ontology)), and a dashed/outlined swatch labeled "Self" (claimed by the user); a circle labeled "Member" and a square labeled "Topic" — **shape**, not fill, is what distinguishes `c:members` from `c:topic` in this diagram, a change from the older circles-only design; fill/outline (green/gray/dashed-outline) is then layered on independently to show who claimed that graph, so a `c:topic` square can carry any of the three colors just as a `c:members` circle can; and a single "Cell" swatch (one uniform rounded-corner, single-border style) — the member-count border-style distinction this legend once carried (three separate "3+-Member Cell"/"2 Member Cell"/"1 Member Cell" entries) was retired project-wide (see Check 10f) — a cell's member count is no longer a visually-checkable fact anywhere in this diagram. None of the legend's names are OWL classes — `category.ttl` defines no `Folder`/`CategoryDefined`/`UserDefined` class; Custom stays a pure filename/display convention. Every circle/square carries an explicit subject-name label (e.g. "Bob", "Self", "BHS") baked directly into the shape — this is how a viewer still learns who is involved, without a separate Subject annotation. There is no "Subject" heading grouping these any more, and no blue per-box Subject text — a cell box no longer displays its subject at all, since it's derivable from the circles/squares already drawn rather than an independently stored fact (see README's Representative Cells section). This diagram illustrates representative cell/category associations generically, not tied to a specific example instance — six boxes, each with a single-line folder-name header (no separate `catType`/`label` split), fill color on the cell's own box (this diagram draws no separate folder icon at all, unlike `folder-mapping.png` — see Check 20). None of the six currently illustrates the gray/Delegate state — that is instead demonstrated by real data in `travel.png`'s "Kyoto Trip 2027" cell (see Check 10's own diagram list):
  - `Medical Appointment` (tan/`Person` fill, green text — folder name matches origin `(Medical Appointment)`'s own label exactly, two squares "Med. Appt mt." — one green/Other, one white/Self, demonstrating `cell:topic`'s unbounded-above cardinality (Check 18) — plus two circles "Self" (white) and "Bob" (green); `cell:TwoMember`)
  - `Friends` (purple/Custom fill, black text, no origin at all, shown as `()`, two circles: a white "Self" member — the cell's required `members` entry, per Check 21 — and a green "Fred", since Fred is the derived subject but not a member; no squares; `cell:TwoMember`)
  - `Employee` (light-blue/`Organization` fill, green text — folder name matches origin `(Employee)`'s own label exactly, one white "Self" circle, no squares; `cell:OneMember`)
  - `Bob Johnson` (tan/`Person` fill, black text — origin `(Others)` ≠ folder name, four circles — two white/Self, two green/Other, all four `c:members` link types filled; no squares; `cell:TwoMember`)
  - `BHS` (tan/`Person` fill, black text — origin `(Affiliations)` ≠ folder name, three circles (Self white, Bob green, BHS green — its three `c:members`) plus one green square (BHS's own organization profile, linked via `cell:topic`) — illustrative only, not tied to real cell-01 data (which has no `cell:topic` value at all, see Check 18); `cell:ThreePlusMember`)
  - `People` (tan/`Person` fill, green text — no origin parenthetical shown, correctly compressed since the origin's label already equals the folder name, one white "Self" circle, no squares; `cell:OneMember`)
...
In `README.md`, `EXAMPLE.md`, and `APP-BEHAVIOR.md`, every mention of a class defined in `persona.ttl` must appear in backticks with the `p:` prefix (e.g. `p:Persona`, `p:Wallet`), every mention of a class or property defined in `graph.ttl` must appear in backticks with the `t:` prefix (e.g. `g:template`, `g:subject`), every mention of a class or property defined in `cell.ttl` must appear in backticks with the `c:` prefix (e.g. `c:Cell`, `c:memberCount`), every mention of a class or property defined in `category.ttl` must appear in backticks with the `cat:` prefix (e.g. `cat:Category`, `cat:templateCell`), every mention of a class or property defined in `other/pets.ttl` must appear in backticks with the `pets:` prefix (e.g. `pets:Pet`, `pets:hasSpecies`), and every mention of a class or property defined in `other/vehicles.ttl` must appear in backticks with the `vehicles:` prefix (e.g. `vehicles:Vehicle`, `vehicles:hasMake`). Every capitalized mention of `Person` (the CCO class) must also appear in backticks. These formatting rules do **not** apply inside headings or subheadings.
