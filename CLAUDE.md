...
- **Other ontologies** (`other/`): small, mostly-vendored peer ontologies — one file per domain — for things a person merely *has* (a pet, potentially a vehicle later) rather than *is*, keeping `persona.ttl`/`persona-templates.ttl` scoped strictly to a person's own identity. `persona.ttl` holds only the thin `hasX` link property for each (e.g. `persona:hasPet`, domain `p:Person`, range that domain's own class), referenced by name with no `owl:imports` in either direction — the same "referenced by name" pattern `cell.ttl` already uses for `cell:origin`'s range (`cat:Category`) and `cell:creator`'s range (`p:Person`/`o:Organization`). Each `other/*.ttl` file's own template classes (e.g. `pets:Pet`, `pets:PetMedicationRecord`) are `rdfs:subClassOf persona:PersonaTemplate`, also referenced by name only, so `graph.ttl`'s `g:template` property keeps one shared range across every domain. The first (and, so far, only) file is `other/pets.ttl` (paired with `other/pets-shacl.ttl`) — see the Core Files table below.
...
| `project_files/ncbitaxon-subset.ttl` | A hand-curated subset of NCBI Taxonomy (NCBITaxon) — a handful of common pet species (cat, dog, goldfish, betta, rabbit, budgerigar) cited by their real upstream IRIs, not a full mirror (NCBITaxon's full distribution is ~2.7 million classes covering all of life). `owl:import`ed by `other/pets.ttl` |
| `project_files/vbo-subset.ttl` | A hand-curated subset of VBO (the Vertebrate Breed Ontology) — a handful of cat/dog breeds plus VBO's own per-species "Mixed Breed" classes, cited by their real upstream IRIs, not a full mirror (VBO's full distribution covers 19,500+ breed concepts across 49 species). CC-BY 4.0 (unlike NCBITaxon/DrOn's CC0/public-domain status) — attribution given in the file's own header and per-class comments. `owl:import`ed by `other/pets.ttl` |
| `cell-templates-shacl.ttl` | Per-template SHACL shapes for birth certificate, driver's license, passport, and medical appointment graph files — `:BirthCertificateDocumentShape`, `:DriversLicenseDocumentShape`, `:PassportDocumentShape`, `:MedicalAppointmentRecordShape` — each directly linked from its `cell-templates.ttl` template cell via `cell:templateShape`; run against the individual graph file, not merged data. The Pets shapes (`:PetMedicationRecordShape`, `:MedicationShape`, `:DosageAmountShape`, `:MedicationAdministrationShape`, and the new `:PetShape`) live in `other/pets-shacl.ttl` instead, paired with `other/pets.ttl` rather than this file |
...
| `project_files/` | Reference materials: imported domain ontologies (PersonOntology.ttl, AddressOntology.ttl, StagingOntology.ttl), BFO/CCO source files, PDFs, docs, plus the hand-curated vendor subsets (`dron-upper.ttl`, `ncbitaxon-subset.ttl`, `vbo-subset.ttl`) `owl:import`ed by `other/pets.ttl` |
...
`graph.ttl`, `cell.ttl`, and `category.ttl` do not carry `owl:versionInfo` at all — these three files are evolving too rapidly right now for a change-log annotation to be worth maintaining. Do not add it to any of the three. They still carry `owl:versionIRI` and `dc:date`; bump those on a real change if convenient, but there's no change-history text to update alongside them. `organization.ttl`, `pdn-identity.ttl`, `cell-templates.ttl`, `persona-templates.ttl`, and each `other/*.ttl` peer ontology (currently just `other/pets.ttl`) are unaffected by this — they still carry `owl:versionInfo` and should keep it updated on a real change, the same way `persona.ttl` does. The `project_files/*.ttl` vendor subsets (`dron-upper.ttl`, `ncbitaxon-subset.ttl`, `vbo-subset.ttl`) also carry `owl:versionInfo`, bumped the same way, even though they aren't listed among the "foundational" files above — a vendor file's own hand-curated subset can grow over time just like any other file here.
