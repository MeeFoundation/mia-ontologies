...
**Compression rule**: if `<local>` is identical to the kebab-cased `<catType>`, the parenthetical is dropped entirely, since it's pure redundancy — `<local>-cat.databook.md` / `<local>-cell[-N].databook.md` — rather than `<local>(<local>)-cat.databook.md`. For example `cat:Work`'s node has local name `work`, and its own `catType` (`Work`) also kebab-cases to `work`, so its files are `work-cat.databook.md`/`work-cell.databook.md`, not `work(work)-cat.databook.md`. This applies only on an exact string match — `acme(organization)-cat.databook.md` keeps its parenthetical since `acme` ≠ `organization`. Most of the top-level tree scaffold compresses this way (`work`, `people`, `others`, `companies`, `finances`, `government`, `state`, `municipality`, `federal`, `ownership`, `affiliations`, `employees`, `immediate-family`, `health-wellness`, `medical`, `banking-payments`), since these nodes' own local name simply *is* their category (there's no more-specific person/organization/thing filed there directly — that's one level further down, e.g. `boston-hub-society(affiliations)` or `acme(organization)`, which don't compress). The same rule applies identically to a topic's `(<containing-cell>)` segment (see [Topic File Naming Convention](#topic-file-naming-convention) below), since that segment is always derived directly from the (possibly-compressed) cell filename.
