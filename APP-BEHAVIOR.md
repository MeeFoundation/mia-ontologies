...
- One special file is the cell's **cell DataBook** (a `.databook.md` file with `type: cell-databook`) — see above for why a folder can never hold more than one. A cell DataBook's *filename* matches its own folder's name verbatim — spaces, case and punctuation included, since the filename feeds no IRI — plus a `(<catType>)` parenthetical where that isn't already redundant (see [Cell DataBook Filename Convention](CLAUDE.md#cell-databook-filename-convention)), and its `title:` field is exactly the folder's own name — the cell's name — updated whenever the folder is renamed. Its `id`, by contrast, is flat, opaque and independent of the folder name — derived at creation for a real cell (see [Cell Id](README.md#cell-id)), and a sequential `cell-<NN>` value in this repo's own example data (see integrity.md's Check 9), the same pattern graph ids already use.
