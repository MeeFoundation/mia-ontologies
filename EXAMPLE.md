...
Alice is a member of the Boston Hub Society, an informal professional networking society. In our example BHS has PDN support in its own server, allowing it to participate directly in this cell alongside Alice and Bob — though an `o:Organization` is never itself a `c:member` subject, so what actually joins is `:BHS_Service`, the `s:ServiceProvider` the society provides, carrying `s:providedBy :BHS`. Alice maintains her BHS profile in [graph 14](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-14>), Bob, another member, keeps his profile updated ([graph 3](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-03>)), and BHS itself contributes the identity it presents as one of the cell's three parties — that graph's subject is `:BHS_Service`, but its claimant is `:BHS`, the organization, since the society is the party really making the claim — in [graph 1](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-01>). BHS also claims the cell's one tool graph — its own organizational profile, recording that the society currently has 80 members and where its website lives ([graph 92](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-92>)). That topic is what makes the cell's derived subject `:BHS` alone, rather than the set of all three member subjects (see integrity.md's Check 18). `cat:Groups` declares a form tool, so its template cell names that shape up front — `c:toolGraphShape oshapes:OrganizationShape` — and every cell filed under it is expected to carry that tool of its own (see integrity.md's Check 31); the *Groups* scaffold cell above it satisfies that with a deliberately empty topic, since its real content lives down here in this leaf cell instead.
...
| Boston Hub Society | Boston Hub Society | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md>) {1} | BHS | `cat:Groups` | 1, 3, 14, 92 |
...
| 11 | [ATT(companies).databook.md](<example/Cells/Companies/ATT/ATT(companies).databook.md#graph-11>) {2}                     | `cat:Companies`    | The ATT cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool), plus an optional organization name and email          | [view](example/graphs/images/graph-11.png) |
...
| 13 | [Boston(home).databook.md](<example/Cells/Home/Previous/Boston/Boston(home).databook.md#graph-13>) {7}               | `cat:Home` | The Boston cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool)          | [view](example/graphs/images/graph-13.png) |
| 14  | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-14>) {1}                     | `cat:Groups` | BHS profile: email, phone and current address                    | [view](example/graphs/images/graph-14.png)|
| 15 | [Drivers License.databook.md](<example/Cells/Government/State/Drivers License/Drivers License.databook.md#graph-15>) {9} | `cat:DriversLicense`      | The Drivers License cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool) | [view](example/graphs/images/graph-15.png) |
| 95 | [Arca(companies).databook.md](<example/Cells/Companies/Arca/Arca(companies).databook.md#graph-95>) {50}               | `cat:Companies`    | The Arca cell's own `:Self` member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool)          | [view](example/graphs/images/graph-95.png) |
| 16 | [Google(companies).databook.md](<example/Cells/Companies/Google/Google(companies).databook.md#graph-16>) {3}               | `cat:Companies`    | The Google cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool), plus an optional organization name and email          | [view](example/graphs/images/graph-16.png) |
| 18 | [Paradise(home).databook.md](<example/Cells/Home/Paradise/Paradise(home).databook.md#graph-18>) {8}           | `cat:Home` | The Paradise cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool) | [view](example/graphs/images/graph-18.png) |
| 19 | [Passport.databook.md](<example/Cells/Government/Federal/Passport/Passport.databook.md#graph-19>) {5}             | `cat:Passport`    | The Passport cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool) | [view](example/graphs/images/graph-19.png) |
...
| 23 | [SSN.databook.md](<example/Cells/Government/Federal/SSN/SSN.databook.md#graph-23>) {6}                     | `cat:SSN`      | The SSN cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool) | [view](example/graphs/images/graph-23.png) |
| 24 | [Birth Certificate.databook.md](<example/Cells/Government/State/Birth Certificate/Birth Certificate.databook.md#graph-24>) {10} | `cat:BirthCertificate`        | The Birth Certificate cell's required member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template declares a tool) | [view](example/graphs/images/graph-24.png) |
...
| #  | Cell DataBook                                                                                     | Category | Key data                                                         | Diagram |
|--- |:-------------------------------------------------------------------------------------------------|:-------------|:-----------------------------------------------------------------|:--------|
| 1  | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-01>) {1}             | `cat:Groups` | BHS's member identity in this cell — `:BHS_Service`, the `s:ServiceProvider` that joins as the member, plus the society's own name and self-description on `:BHS`; claimed by BHS, the organization | [view](example/graphs/images/graph-01.png) |
...
| 3  | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-03>) {1}                     | `cat:Groups` | Bob's BHS member persona (name, email, phone, address)          | [view](example/graphs/images/graph-03.png) |
| 92 | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-92>) {1} | `cat:Groups` | BHS's own organizational profile — its current member count (`o:numMembers` 80) and public website (`o:hasWebsite`) — the cell's manually-added tool graph, claimed by BHS | [view](example/graphs/images/graph-92.png) |
...
`c:CGraph`'s `c:claimant` and `c:SCGraph`'s `c:graphSubject`), plus `shacl/persona-shacl.ttl`,
...
sync. Since `c:formShape`'s range is `sh:NodeShape` (`cell.ttl`), the value already *names the shape
