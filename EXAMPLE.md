...
Alice also has a cat, Ginger. Under a *Pets* category folder she has a *Ginger* folder for this specific pet — a user-defined instance folder reusing its parent's own `cat:Pets` origin, and within it a *Health* subfolder, and within that a *Medications* cell recording Ginger's medications: a completed course of amoxicillin/clavulanate (brand name Clavamox, from Zoetis) and an ongoing daily glucosamine/chondroitin joint supplement ([topic 32](<example/Cells/Pets/Ginger/Health/Medications/Medications.databook.md#topic-32>)). 
