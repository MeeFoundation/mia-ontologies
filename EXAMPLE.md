...
| 95 | [Arca(companies).databook.md](<example/Cells/Companies/Arca/Arca(companies).databook.md#graph-95>) {50}               | `cat:Companies`    | The Arca cell's own `:Self` member entry — carries her given name (required by `ContactInfoShape`, `c:memberGraphShape`, since this template is `isTopicCell: true`)          | [view](example/graphs/images/graph-95.png) |
...
| 92 | [Boston Hub Society(groups).databook.md](<example/Cells/Groups/Boston Hub Society/Boston Hub Society(groups).databook.md#graph-92>) {1} | `cat:Groups` | BHS's own organizational profile — its current member count (`o:numMembers` 80) and public website (`o:hasWebsite`) — the cell's manually-added `c:topic`, claimed by BHS | [view](example/graphs/images/graph-92.png) |
