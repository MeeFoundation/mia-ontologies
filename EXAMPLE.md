...
This cell is also the example's one use of a **built-in tag**: it carries `c:builtInTag "LoyaltyProgram"`, so searching that label returns a link to it — and would return every other provider cell she tags the same way, wherever each sits in her tree. See [Tags](APP-BEHAVIOR.md#tags) and [Finding Cells by Tag](APP-BEHAVIOR.md#finding-cells-by-tag) in APP-BEHAVIOR.md.
