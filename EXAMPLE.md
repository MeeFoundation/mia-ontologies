...
What sets the *Arca* cell apart from *Google* and *ATT* structurally is its second `c:member` entry. `:Arca_Backup`, the `s:BackupService` Arca provides, joins the cell as a real member with its own self-claimed graph ([graph 96](<example/Cells/Companies/Arca/Arca(companies).databook.md#graph-96>)), carrying `s:providedBy :Arca`. Two distinct members — Alice ([graph 95](<example/Cells/Companies/Arca/Arca(companies).databook.md#graph-95>)) and the backup service — make this a two-member cell, where the *Google* and *ATT* cells each have only Alice.

A `s:BackupService` differs from the `s:ServiceProvider` in the *Citibank* and *Boston Hub Society* cells in one structural way and from the `s:AIAgentService` in *Kyoto Trip 2027* in another. Unlike a provider service, no organization claims on its behalf: `:Arca` is named only as its `s:providedBy` value, and the service claims its own member graph directly. Unlike an AI agent, it carries no `s:actsFor` value at all — a backup service is nobody's delegate, it serves the cell itself, so there is no single principal to name. Like every service, it can never be a `c:creator` or a `c:owner`: Alice alone created and owns this cell.
...
| #  | Cell DataBook                                                                                     | Category | Key data                                                         | Diagram |
|--- |:-------------------------------------------------------------------------------------------------|:-------------|:-----------------------------------------------------------------|:--------|
| 1  | [Boston Hub Society(affiliations).databook.md](<example/Cells/Affiliations/Boston Hub Society/Boston Hub Society(affiliations).databook.md#graph-01>) {1}             | `cat:Affiliations` | BHS's member identity in this cell — `:BHS_Service`, the `s:ServiceProvider` that joins as the member, plus the society's own name and self-description on `:BHS`; claimed by BHS, the organization | [view](example/graphs/images/graph-01.png) |
...
| 96 | [Arca(companies).databook.md](<example/Cells/Companies/Arca/Arca(companies).databook.md#graph-96>) {50} | `cat:Companies` | Arca's backup service as a real cell member — `:Arca_Backup`, typed `s:BackupService`, carrying `s:providedBy :Arca`; claimed by the service itself, since no organization claims on a backup service's behalf | [view](example/graphs/images/graph-96.png) |
...
| 27 | [Citibank(banking-payments).databook.md](<example/Cells/Finances/Banking & Payments Firms/Citibank/Citibank(banking-payments).databook.md#graph-27>) {4} | `cat:BankingPayments` | The Citibank cell's other required `member` entry — `:Citibank_Service`, the `s:ServiceProvider` that joins as the member, plus Alice's own self-claimed notes about Citibank as an institution, asserted on `:Citibank`; claimed by Alice, alongside Citibank's own claimed record about her (graph 76) | [view](example/graphs/images/graph-27.png) |
...
| 67 | [Kyoto Trip 2027(trips).databook.md](<example/Cells/Travel/Trips/Kyoto Trip 2027/Kyoto Trip 2027(trips).databook.md#graph-67>) {47} | `cat:Trips` | Alice's travel agent's own self-claimed member graph — typed `s:AIAgentService`, carrying `s:actsFor :Self` | [view](example/graphs/images/graph-67.png)|
