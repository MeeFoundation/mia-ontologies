...
Alice also has a cat, Ginger. Under a *Pets* category folder she has a *Ginger* folder for this specific pet — a user-defined instance folder reusing its parent's own `cat:Pets` origin, the same pattern "Bob Johnson" uses under "Others" — and within it a *Health* subfolder, and within that a *Medications* cell recording Ginger's medications: a completed course of amoxicillin/clavulanate (brand name Clavamox, from Zoetis) and an ongoing daily glucosamine/chondroitin joint supplement ([topic 32](<example/Cells/Pets/Ginger/Health/Medications/Medications.databook.md#topic-32>)). Both the *Ginger* cell itself and the *Medications* cell beneath it are *Single-Member* cells whose subject is Ginger — she isn't a Mia user and has no `p:Person` individual of her own, so wherever her own topic appears it's linked as an `otherTopic` rather than the required `memberTopic`; that slot instead holds a topic about Alice herself ([topic 36](<example/Cells/Pets/Ginger/Ginger(pets).databook.md#topic-36>) for the *Ginger* cell — deliberately empty, since `memberTopics`' requirement is about `t:subject`/`t:claimant`, not content; [topic 33](<example/Cells/Pets/Ginger/Health/Medications/Medications.databook.md#topic-33>) for *Medications*). The *Ginger* cell's own `otherTopic` about Ginger ([topic 37](<example/Cells/Pets/Ginger/Ginger(pets).databook.md#topic-37>)) is just enough to back its `subject: ":Ginger"` with a real topic.
