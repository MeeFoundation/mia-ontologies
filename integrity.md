...
for f in glob.glob('example/Cells/**/*.databook.md', recursive=True):
    ...
    if not mia.get('member'):
        continue
    # mia.member/topic entries carry their own about-ness value directly — no
    # separate mia.graphs list to look up — but under different keys, since the
...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    # mia.userTag — a plain string, so all three original failure modes
    # still apply to it unchanged.
    if 'userTag' in mia:
        values = as_list(mia.get('userTag'))
        if not values:
    ...
    # mia.serviceTag — a nested namespace/key/value mapping per entry.
    ...
    # sees.
    if 'serviceTag' in mia:
        entries = as_list(mia.get('serviceTag'))
        if not entries:
