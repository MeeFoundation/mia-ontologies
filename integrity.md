...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    for field, entry in entries:
        ...
        if not entry.get('claimant'):
            ...
            errors += 1
        # A member entry is a cell:MemberGraph and carries graphSubject; a tool's
...
- **13c** — the one arrow off `MemberGraph` (`graphSubject`) has `rdfs:domain cell:MemberGraph`, and its target must match its actual `rdfs:range`, any resource IRI (`xsd:anyURI`), not a Person/Organization union. `ToolGraph` carries **no** about-ness arrow of its own: what a tool's graphs are about is the holding tool's single `toolTopic`, drawn in `cell.png` (Check 12) rather than here. Neither leaf may show the other's arrow — the two are `owl:disjointWith`, and `shacl/cell-shacl.ttl`'s `:ToolGraphShape` carries an `sh:maxCount 0` on `cell:graphSubject` to enforce it.
...
- **13e** — the hierarchy `Graph` → `CGraph` → `MemberGraph`/`ToolGraph` shown in the diagram matches `cell.ttl`'s actual `rdfs:subClassOf` relationships by class local name, and the two leaves carry `owl:disjointWith` one another. `Graph` and `CGraph` are both `cell:abstract true`; the two leaves are not.
- **13f** — the class-label colors match `cell.ttl`'s `cell:abstract` annotations: `Graph` and `CGraph` blue (both `cell:abstract true`), `MemberGraph` and `ToolGraph` black (neither carries it). The Key box lists exactly the swatches the diagram actually uses — two here, since nothing in this diagram is hidden from the user; do not paste `cell.png`'s third, green swatch in unless a hidden class is ever added.
- **13g** — no subclasses appear below `MemberGraph` or `ToolGraph` — `cell.ttl` defines none for either. If any appear here or in `cell.ttl`, reconcile them. Check also that `cell:member`'s `rdfs:range` is `cell:MemberGraph` and `cell:toolGraph`'s is `cell:ToolGraph` (the ranges are what type a real graph, since nothing in a `v4.member` entry or a tool's own graph marks its own kind — see Check 3).
...
**Check 27 — a `member` graph's `shape:` value, and a declared tool's graph's, must be among its cell's own `TemplateCell`'s `memberShape`/`toolShape` values**: Check 26 only compares a graph's `shape:` value against its own body content — this check goes one level higher, comparing it against what the cell's *category* itself declares as authoritative. For every cell-databook under `example/Cells/` (excluding `under-development/`) whose `v4.category` value names a concept with a matching `cell:TemplateCell` in `cat-templates.ttl` (reverse lookup, per Check 18/README's Lazy Instantiation pattern — a category with no such template has nothing to check here; a defensive branch, not a live case today, since Check 29 guarantees every `category.ttl` concept except `cat:Person`/`cat:Organization` has one): every `v4.member`-list graph carrying a `shape:` value must have that value be one of the TemplateCell's own `cell:memberShape` value(s); and, **only when that TemplateCell declares a tool at all**, every tool graph carrying a `shape:` value must be among the declared tool's own `cell:toolShape` value(s) — direct CURIE-set membership, since `cell:shape`'s range is `sh:NodeShape` (`cell.ttl`), the same range `cell:memberShape`/`cell:toolShape` already carry, so no `sh:targetClass` resolution and no named exemptions are needed any more. A tool on a cell whose category declares none is **not** checked against any shape at all: any cell may gain a tool its category never declared, and the user picks that tool's shape themselves — from the app's full list of shapes, not from anything the cell's category declares — so the category's own `TemplateCell` has no authority over it. A category that declares no tool offers no shape hint either. This is not itself an OWL/SHACL-expressible constraint (same reasoning as Checks 18/21/23/25/26 — it requires dereferencing values across `cat-templates.ttl`, not just cell-databook YAML), so it's checked here instead. Run:
...
for block in re.split(r'\n\n(?=ctpl:)', read('cat-templates.ttl')):
    ...
    def shape_values(prop, block):
        ...
        # cell:memberShape/cell:toolShape are 0..N, so a template may
    ...
    # A declared tool is a bracketed blank node carrying cell:toolShape;
    ...
    category_shapes[cat_local] = {
        ...
        'tool': shape_values('cell:toolShape', block),
...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    for field in ('member', 'tool'):
        ...
        for entry in entries_for(field):
            ...
            prop = 'memberShape' if field == 'member' else 'toolShape'
...
**Check 38 — `cell.ttl`'s `cell:Tool` hierarchy matches `images/cell-ontology/tool.png`**: The tool hierarchy has its own diagram for the same reason `tag.png` does — `cell:Tool` has no `rdfs:subClassOf` relation to `cell:Cell`, so it is not part of that hierarchy and is only pointed at from it. `Tool` (abstract, blue) sits at the top with its three concrete children `FormTool`, `CalendarTool` and `CanvasTool` (all black) below it:

- **38a** — the two arrows off `Tool` (`toolTopic`, `toolShape`) each have `rdfs:domain cell:Tool`; `toolTopic`'s target is `xsd:anyURI` at **1..1** (required exactly once on a live tool — `shacl/cell-shacl.ttl`'s `:LiveToolShape`), and `toolShape`'s is `sh:NodeShape` at 0..N. Both are drawn off `Tool`, not off `FormTool`: `toolTopic` is equally meaningful for every kind of tool, and `toolShape` is carried in the declared-tool role, which any tool class can occupy.
- **38b** — the one arrow off `FormTool` (`toolGraph`) has `rdfs:domain cell:FormTool` and targets `cell:ToolGraph` at **1..N**. It hangs off `FormTool` alone, never off `Tool`: it is the single property that distinguishes the one tool kind with content today from the two stubs. OWL/SHACL itself sets no maximum — the real upper bound, one graph per cell member each with a distinct claimant, is Check 25, not something this diagram draws.
- **38c** — `CalendarTool` and `CanvasTool` carry no arrows of their own: `cell.ttl` defines no property with either as its domain. They are drawn all the same, so the extension point reads as plural rather than as a `FormTool` that happens to be abstract's only child.
- **38d** — the hierarchy `Tool` → `FormTool`/`CalendarTool`/`CanvasTool` matches `cell.ttl`'s actual `rdfs:subClassOf` relationships by class local name. The three leaves are **not** `owl:disjointWith` one another (unlike `cell:MemberGraph`/`cell:ToolGraph`, and unlike `cell:TemplateCell`/`cell:InstanceCell`), so no disjointness is drawn between them.
- **38e** — the class-label colors match `cell.ttl`'s `cell:abstract` annotations: `Tool` blue (`cell:abstract true`), the three leaves black (none carries it). The Key lists exactly the swatches this diagram uses — two here, since nothing in it is the green "concrete but hidden" case.
