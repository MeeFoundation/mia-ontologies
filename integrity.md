...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    # mia.integrationTag — a nested namespace/key/value mapping per entry.
    # The namespace and key *spellings* are :IntegrationTagShape's business
    # (sh:pattern); what is checked here is only the YAML shape SHACL never
    # sees.
    if 'integrationTag' in mia:
        entries = as_list(mia.get('integrationTag'))
        if not entries:
            print(f'VIOLATION {path}: integrationTag is present but empty — omit the field instead')
            violations += 1
        seen = set()
        for entry in entries:
            if not isinstance(entry, dict):
                print(f'VIOLATION {path}: integrationTag entry {entry!r} is not a mapping — '
                      f'it needs namespace/key/value sub-keys; the flat string form is retired')
                violations += 1
                continue
            extra = sorted(set(entry) - set(TAG_SUBKEYS))
            if extra:
                print(f'VIOLATION {path}: integrationTag entry has unrecognized sub-key(s) {extra}')
                violations += 1
            malformed = False
            for sub in TAG_SUBKEYS:
                if sub not in entry:
                    print(f'VIOLATION {path}: integrationTag entry is missing {sub}')
                    violations += 1
                    malformed = True
                    continue
                v = entry[sub]
                if not isinstance(v, str):
                    print(f'VIOLATION {path}: integrationTag {sub} {v!r} is not a string (quote it)')
                    violations += 1
                    malformed = True
                elif not v.strip():
                    print(f'VIOLATION {path}: integrationTag {sub} is empty/blank')
                    violations += 1
                    malformed = True
            if malformed:
                continue
            ...
            if triple in seen:
                print(f'VIOLATION {path}: integrationTag {triple!r} is duplicated on this cell')
                violations += 1
            seen.add(triple)
