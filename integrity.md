...
for path in sorted(glob.glob('example/Cells/**/*.databook.md', recursive=True)):
    ...
    for i, tool in enumerate(as_list(v4.get('tool'))):
        ...
        if tool.get('type') not in ('form', 'calendar', 'canvas'):
            ...
            errors += 1
        # cell:formTopic and cell:formGraph are scoped to cell:FormTool
        # (cell.ttl), so both are required of a form tool and of nothing
        # else; the two stub kinds carry no content properties at all yet.
...
If a malformed entry is found, fix it directly (turn a stray bare string back into a full mapping, correct the id, or add the missing `claimant`/`graphSubject`/`formTopic`).
...
- **13c** — the one arrow off `MemberGraph` (`graphSubject`) has `rdfs:domain cell:MemberGraph`, and its target must match its actual `rdfs:range`, any resource IRI (`xsd:anyURI`), not a Person/Organization union. `ToolGraph` carries **no** about-ness arrow of its own: what a tool's graphs are about is the holding tool's single `formTopic`, drawn in `cell.png` (Check 12) rather than here. Neither leaf may show the other's arrow — the two are `owl:disjointWith`, and `shacl/cell-shacl.ttl`'s `:ToolGraphShape` carries an `sh:maxCount 0` on `cell:graphSubject` to enforce it.
...
for f in glob.glob('example/Cells/**/*.databook.md', recursive=True):
    ...
    # levels (cell:graphSubject on the graph vs cell:formTopic on the tool).
    pt = v4.get('member') or []
    pt = pt if isinstance(pt, list) else [pt]
    tools = v4.get('tool') or []
    tools = tools if isinstance(tools, list) else [tools]
    pt_subs = {t.get('graphSubject') for t in pt}
    # cell:formTopic is scoped to cell:FormTool, so only a form tool
...
**Check 21 — `:Self` must be a member of every cell in the user's own tree**: A cell-databook under `example/Cells/` — the user's own instance tree — can only ever have gotten there one of two ways: (1) the user created it themselves, in which case they (`:Self`) are trivially a member, or (2) someone else shared it with the user, in which case the share necessarily made `:Self` a member (a cell can't be "shared with" someone without them becoming a member of it). Either way, `:Self` must be one of the cell's active members — i.e. `:Self` must be the `cell:graphSubject` of at least one of that cell's `member` — for **every** cell in `example/Cells/`, regardless of how many members the cell has or what the cell's derived subject (Check 18) is. This is strictest for a cell with only one `member` entry: that entry's subject must be `:Self`, full stop — never a tool's own `cell:formTopic` (see Check 18's placement rule above), even when the cell's derived-from-tool subject is a third party (e.g. `Jane_Starostina`, `Sophia_Walker`, `Ginger`) and no other graph happens to exist yet. Cells with more members have more room, so `:Self` just needs to be one of the members alongside whichever other real members the cell has (already satisfied by every existing example, e.g. Bob Johnson, Fred Flintstone, Medical Appointment, Boston Hub Society). This is not itself an OWL/SHACL-expressible constraint (same reasoning as Check 18 — it requires dereferencing each `member` value's own `graphSubject`, not just counting or matching cardinalities), so it's checked here instead. Run:
...
- **38a** — **no arrow is drawn off `Tool` at all.** `cell.ttl` defines no property with `rdfs:domain cell:Tool`: all three tool-content properties are scoped to `cell:FormTool`, the one tool class with a content model. An arrow drawn off the abstract class would assert of `CalendarTool` and `CanvasTool` something neither has a content model for yet.
- **38b** — all three arrows hang off `FormTool`, each with `rdfs:domain cell:FormTool`: `formTopic` to an `xsd:anyURI` box at **1..1** (required exactly once on a live form tool — `shacl/cell-shacl.ttl`'s `:LiveToolShape`, conditioned on the class), `formGraph` to a `cell:ToolGraph` box at **1..N**, and `formShape` to a `sh:NodeShape` box at 0..N. OWL/SHACL sets no maximum on `formGraph` — the real upper bound, one graph per cell member each with a distinct claimant, is Check 25, not something this diagram draws.
- **38c** — `CalendarTool` and `CanvasTool` carry no arrows: `cell.ttl` defines no property with either as its domain, and they inherit none, since `Tool` carries none either. They are drawn all the same, so the extension point reads as plural rather than as a `FormTool` that happens to be abstract's only child.
