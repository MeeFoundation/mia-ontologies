---
id: http://www.example.org/mia/categories/bob-johnson(others)-cell
title: "Bob Johnson (Cell)"
type: cell-databook
version: 1.0.6
created: 2026-07-10
description: >
  Cell DataBook of category "Bob Johnson" (mia.catType: Others). Content may include secondary/folder/note links, and may carry a required primary topic.
mia:
  creator: ":Self"
  parties: "cell:TwoParty"
  primary: "https://www.example.org/mia/topics/bob-johnson.bob-johnson(bob-johnson)(others)(02)"
  secondary:
    - "https://www.example.org/mia/topics/self.self(bob-johnson)(others)(12)"
    - "https://www.example.org/mia/topics/bob-johnson.self(bob-johnson)(others)(04)"
    - "https://www.example.org/mia/topics/self.bob-johnson(bob-johnson)(others)(08)"
---
